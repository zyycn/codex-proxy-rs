...
- `client`：`/v1` 路由、Bearer API Key 鉴权、Responses 请求解析和 SSE 输出。
...
一次 `POST /v1/responses` 请求经过以下步骤：

1. 请求 ID 中间件建立全链路标识，访问日志记录基础 HTTP 信息。
2. `client::auth` 从 `Authorization: Bearer sk_...` 取出完整 Key，并在 PostgreSQL 校验是否存在且启用。
3. API 层解析请求，提取客户端 IP、User-Agent 和 transport-only 参数。
...
流式请求只有在响应尚未提交给客户端时才能透明恢复或换号。首段预取会识别终止事件和账号级失败；一旦真实输出已经发给客户端，后续断流只能以 SSE 失败结束，不能在另一账号上重放后拼接到已有输出。
