...
| L1 | `keys` | 客户端 API key 域：PG 明文 store、唯一索引鉴权点查、管理操作 | 不得建立进程内鉴权缓存；完整 key 只向已认证管理端返回 |
...
| L2 | `fleet` | 上游账号机队域：实体、池、调度器、store、quota、token 刷新、Cookie、导入导出、管理操作 | — |
...
| 允许的同层/特殊依赖 | 理由 |
| --- | --- |
| `fleet` → `models` | 候选过滤需要模型允许列表；单向 |
| `fleet` → `settings` | 池参数/刷新策略订阅设置快照（watch 类型）；单向 |
| `auth` → `settings` | admin API key 哈希存放在 runtime_settings 表，经 settings 的 store 方法读取；单向 |

**全库硬禁**（迁移验收时由 §9 临时审计断言）：

- L1–L3 任何模块 import `crate::api` / `crate::bootstrap`。
- `dispatch`、`fleet`、`telemetry` 等领域模块 import `axum`。
- `api` 下出现 `sqlx` 或 `redis`（SQL 与 Redis 命令都只允许在各域的 `store` 文件里，§5.2）。
- `upstream` import `crate::{fleet, dispatch, telemetry, keys, auth, settings, update, models}`。
- 任何模块 import 旧路径 `crate::admin` / `crate::proxy` / `crate::runtime` / `crate::web` / `crate::http` / `crate::config`（改造完成后这些模块不存在）。

### 2.3 控制反转两处（消除向上依赖）

1. **设置传播**：`SettingsService` 只暴露 `tokio::sync::watch::Receiver<SettingsSnapshot>`；`fleet`（池、刷新策略）、`models`（别名）各自订阅。装配在 `bootstrap` 完成。
...
│       ├── token_client.rs
│       ├── protocol/
│       │   ├── mod.rs  schema.rs  responses.rs  events.rs  sse.rs
│       │   ├── websocket.rs  websocket_errors.rs
│       ├── transport/
│       │   ├── mod.rs  client.rs  client_sse.rs  endpoints.rs  headers.rs  tls.rs
│       │   ├── response_meta.rs  usage.rs  diagnostics.rs
│       │   ├── websocket.rs  websocket_frames.rs
│       │   ├── websocket_pool.rs  websocket_pump.rs
│       └── fingerprint/
...
├── fleet/                        # L2 上游账号机队域（原 upstream/accounts + upstream/scheduler + admin/accounts/service）
│   ├── mod.rs
│   ├── account.rs                # 实体（原 model.rs）
│   ├── window.rs
│   ├── pool/
│   │   ├── mod.rs  state.rs  filters.rs   # mod.rs = 池门面；状态与过滤按 §5.4 拆分
│   ├── scheduler/
│   │   ├── mod.rs  candidates.rs  feedback.rs   # mod.rs = AccountScheduler 门面
│   │   └── strategy/
│   │       ├── mod.rs  smart.rs  quota_reset.rs  round_robin.rs  sticky.rs
│   ├── store/
│   │   ├── mod.rs  queries.rs  rows.rs  write.rs   # mod.rs = AccountStore trait + PG 门面
│   ├── quota/
...
│   │   ├── mod.rs  types.rs  store.rs  service.rs  resolve.rs   # store = Redis 键 + 二级索引（§4B.3），无内存映射副本
│   ├── recovery/
...
│       ├── accounts_routes/
│       │   ├── mod.rs  query.rs  lifecycle_routes.rs  oauth_routes.rs  quota_routes.rs
│       │   ├── import_routes.rs  export_routes.rs  probe_routes.rs
│       ├── keys_routes.rs
...
└── bootstrap/                    # L5 进程装配（原 runtime + config/schema,loader）
    ...
    ├── import_sqlite/
    │   ├── mod.rs  core.rs  telemetry.rs   # SQLite v3 → PG；§5.5 唯一跨域写豁免
    └── tasks/
...
2. **实体集合域用复数**（`models`、`keys`、`buckets`）；**能力、抽象或集合体域用单数**（`fleet`、`dispatch`、`telemetry`、`auth`、`settings`、`update`、`upstream`、`api`、`infra`、`bootstrap`）。
3. 子目录表达子能力（`scheduler/strategy/`），不表达文件类型——禁止 `services/`、`stores/`、`utils/`、`helpers/`、`common/`、`misc/` 这类"按技术角色装桶"的目录。

### 4.2 文件名

1. snake_case。**主类型 = 文件名的 PascalCase**（`account.rs` ↔ `Account`）；目录模块的主门面放在 `mod.rs`（`scheduler/mod.rs` ↔ `AccountScheduler`）。
2. **角色后缀词表**（一个文件一个角色）：

| 文件名 | 角色 | 约束 |
| --- | --- | --- |
| `router.rs` | 路由组合 | 只 merge/nest，不写 handler |
| `*_routes.rs` / `routes.rs` / `*_routes/mod.rs` | HTTP handler + 请求/响应 DTO | 不写 SQL、不含领域规则 |
| `service.rs` | 领域服务（该域对外门面） | 不含 SQL 文本与 Redis 命令 |
| `store.rs` / `store/mod.rs` | 持久化/运行态存取（PG SQL 或 Redis 命令） | SQL 与 Redis 命令只允许出现在这里；类型名 `Pg*Store` / `Redis*Store` |
...
| `errors.rs` | 该域错误类型 | — |
| `mod.rs` | 目录模块入口 | 可承载该模块门面；与其他 `.rs` 一样遵守 §5.4 的 400/800 行预算 |
...
7. 有子模块的模块统一使用 `foo/mod.rs`，禁止 `foo.rs` 与 `foo/` 并存；无子模块的叶子模块继续使用 `foo.rs`。
...
有子模块的模块统一以目录下 `mod.rs` 为入口；禁止同时存在 `foo.rs` 与 `foo/`。`mod.rs` 可以承载该模块的对外门面、共享 DTO 或路由 handler，但不得成为无边界聚合文件：声明/re-export 之外的实现同样受 §5.2–§5.4 的职责边界与 400/800 行预算约束。叶子模块使用同级 `foo.rs`，不为单文件额外套目录。
...
- 软上限 **400 行**（超过即在 review 中说明理由）；硬上限 **800 行**（超过必须拆，§9 临时审计直接失败）。
...
| `admin/accounts/routes.rs` | 1455 | → `api/admin/accounts_routes/mod.rs` 按子资源拆（accounts / oauth / quota / import-export） |
| `upstream/accounts/pool.rs` | 1420 | → `fleet/pool/mod.rs`（门面）+ `pool/state.rs`（槽位/在途）+ `pool/filters.rs` |
| `upstream/transport/client.rs` | 1251 | → `client.rs` + `client_sse.rs`（SSE 请求路径） |
| `upstream/accounts/store/mod.rs` | 1126 | → `fleet/store/mod.rs` 门面 + `{queries,rows,write}.rs` 按读写与行映射拆分 |
...
| `upstream/accounts/token_refresh/runtime.rs` | 953 | → `fleet/refresh/{service,lease,policy}.rs` |
| `admin/system/updater/mod.rs` | 811 | → `update/{service,state}.rs` |

新增的 `bootstrap/import_sqlite/mod.rs` 按导入表组拆出 `{core,telemetry}.rs`，避免一次性迁移命令突破 800 行硬线；它仍是 §5.5 唯一允许跨域写入的入口。
...
| accounts / account_cookies | `fleet` |
...
| `cpr:lease:refresh:*` | `fleet`（refresh/lease.rs） |
| `cpr:affinity:*` | `dispatch`（affinity/store.rs） |
| `cpr:models:plan_snapshots` | `models` |

跨域读通过属主域的 query/service 方法，不直接跨域写。**唯一豁免**：`bootstrap/import_sqlite/mod.rs`（一次性导入命令，database.md §6）按依赖序直写全部 PG 表；v3 源库退役后该模块可删。
...
超 800 行文件 11 个（§5.4 已列）；模块入口风格曾混用 `foo.rs + foo/` 与 `foo/mod.rs`，终态按 §5.1 统一。
...
| —（新增） | `bootstrap/import_sqlite/mod.rs`（SQLite v3 → PG 导入命令，吸收原 0004 迁移草案） |
...
| `admin/keys/service.rs` | `keys/{types,store,service,manage}.rs` †（明文内存缓存消亡，database.md §4.3） |
...
| `admin/accounts/routes.rs` | `api/admin/accounts_routes/mod.rs` †（按子资源拆） |
| `admin/accounts/quota_view.rs` | `fleet/manage/quota_view.rs` |
| `admin/accounts/service/*` | `fleet/manage/*`（`importing`→`import`、`exporting`→`export`、`testing`→`probe`、`contracts`→`types`） |
...
| `upstream/accounts/model.rs` | `fleet/account.rs` |
| `upstream/accounts/{pool,window}.rs` | `fleet/pool/mod.rs` + `fleet/window.rs`（pool 拆 †） |
| `upstream/accounts/store/*` | `fleet/store/{mod,queries,rows,write}.rs` † |
| `upstream/accounts/quota/*` | `fleet/quota/{service,runtime}.rs` † |
| `upstream/accounts/token_refresh/*` | `fleet/refresh/{policy,lease,service}.rs` † |
| `upstream/accounts/cookies.rs` | `fleet/cookies/store.rs` |
| `upstream/accounts/importing.rs` | `fleet/import.rs` |
| `upstream/accounts/service.rs` | 并入 `fleet/mod.rs` 门面导出（32 行） |
| `upstream/scheduler/*` | `fleet/scheduler/{mod,candidates,feedback,strategy/*}.rs` |
...
## 8. 执行计划（PR0 + 六个顺序搬家 PR，每个独立编译绿、测试绿）

每个 PR 的固定验收：`cargo fmt --check`、`cargo clippy -- -D warnings`、`cargo test`、§9 临时架构审计、`backend/tests` 镜像目录同步搬移。**PR0 是唯一的行为变更 PR**（存储引擎与 schema 语义一次到位）；PR1 起全部为零行为搬家（更名与移动，不改逻辑）。

| PR | 内容 | 关键动作 |
| --- | --- | --- |
| **PR0** | **存储终态切换**（database.md §7 全部 12 条，在现有目录结构内完成） | Cargo：sqlx 加 postgres、新增 redis；`infra/database.rs` 改 PG + 0001 终态基线、新增 `infra/redis.rs`；全部 store 换 PG 方言 / 运行态四键空间改 Redis；成功/失败事实拆表 + provider 维度 + 两条写入事务；客户端 key 明文可取回、session/admin-key 哈希化 + `client_api_key_id` 归因贯通；Dashboard 口径换源；trim 周期任务化；`import-sqlite` / `rebuild-buckets` 子命令；tests support 换 PG+Redis;deploy compose 加双服务。验收对齐 database.md §6.4 |
...
| **PR3** | `fleet` / `models` 域成立 | `upstream/accounts`、`upstream/scheduler`、`admin/accounts/service` 三处合并为 `fleet`；`upstream` 缩为纯客户端并落成 `openai/` 子目录；动名词文件更名；Repository→Store |
...
| **PR6** | 行数预算清账 | §5.4 清单内 11 个文件拆到硬线以下；纳入 §9 临时审计 |

顺序依据：存储切换触及所有 store、无法按域切片，必须整体先行（PR0）——先切引擎再搬家，每个 store 文件只被"重写"一次、"移动"一次，不叠加；PR1–4 自底向上消除旧模块的存在必要；PR5 收壳；PR6 纯内部拆分无跨域影响。原"PR1 与数据库改造互锁"的约定由 PR0 替代，PR2 中的 session/admin-key 哈希化条目亦已并入 PR0；客户端 API key 保持 database.md §4.3 的明文可取回设计。

---

## 9. 临时架构审计

迁移验收期间可从历史提交临时取出以下门禁逻辑在本地执行，用于检查依赖方向、命名和行数预算。该脚本不恢复到仓库，也不加入 CI；正式门禁仍由现有编译、测试与 Clippy 工作流承担。
...
deny "domain -> api/bootstrap" 'crate::(api|bootstrap)\b' \
  ...
  src/fleet src/models src/dispatch
# upstream 是纯客户端
deny "upstream -> domain" 'crate::(fleet|dispatch|telemetry|keys|auth|settings|update|models)\b' src/upstream
# HTTP 类型只在 api / bootstrap
deny "axum outside api" '\baxum(::|\b)' \
  src/infra src/upstream src/telemetry src/keys src/auth src/settings src/update \
  src/fleet src/models src/dispatch
# SQL 与 Redis 命令只在 store / query
deny "sqlx outside store/query" 'sqlx::query' \
  $(find src/api -name '*.rs')
deny "redis outside store" '\bredis::' \
  $(find src/api -name '*.rs')
# 旧模块根不得复活
...
# 模块入口统一：禁止 foo.rs + foo/ 并存
while read -r f; do
  [ -d "${f%.rs}" ] && { echo "ARCH VIOLATION: mixed module entry: $f + ${f%.rs}/" >&2; fail=1; }
done < <(find src -name '*.rs')
# 行数硬上限
while read -r f; do
  lines=$(wc -l < "$f")
  [ "$lines" -gt 800 ] && { echo "ARCH VIOLATION: $f has $lines lines (> 800)" >&2; fail=1; }
done < <(find src -name '*.rs')
exit $fail
```

临时审计对迁移完成后的全部目录执行；检查结束后不保留生成的脚本文件。
...
- `docs/` 保留权威文档 `architecture.md`（本文）、`database.md`，以及随改造关闭条目的 `naming-review.md`。行为细节（调度策略、WS 池、更新流程等）以模块文档注释为准（`scheduler/mod.rs` 是范本），不再维护第二份行为说明。
- `deploy/` 随 PR0 扩容（compose 增 postgres/redis 服务与健康检查依赖）；`release/`、`skills/` 现状合格，不动。
- 前端联动（全部随 PR0）：已认证管理端的 keys 列表继续返回完整 key，并保留长期复制与 CCSwitch 导入；admin API key 状态不再有脱敏值（哈希不可逆，只余"已启用"）；成功记录响应不再有 `level` 字段；错误明细/筛选改走 `/api/admin/ops/errors`；Dashboard "总请求/总 token" 卡片改为 account_usage 来源并标注"成功累计"。
...
| `fleet` | `scheduler/candidates` 加 provider 硬过滤（单池 + 过滤，不按 provider 拆池）；`manage/{oauth,import}` 按 provider 分实现（CF 导入格式 = `(account_id, api_token)`）；refresh/租约对 CF 账号整体跳过（`refresh_token` NULL 语义已覆盖） |
