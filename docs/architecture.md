...
view / component
      ...
      -> api module / api stream adapter
      -> request transport
```

- `api/request.ts` 返回 `unknown`，endpoint module 使用 Zod 在 HTTP 边界解析；不使用类型断言代替运行时校验。
- 401 处理由 `main.ts` 注入，request transport 不依赖 router 或 store。
- EventSource、URL 和事件解析只存在于 `api/streams`；feature 只消费 typed event 和可关闭 stream handle。
