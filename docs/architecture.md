...
后端生产代码只放在 `backend/src/`。后端测试统一放在 `backend/tests/`，并按生产边界分层组织；`backend/src/` 禁止出现 `#[cfg(test)]`、`mod tests` 和 test-only helper。
...
- `service.rs`、`stream/lifecycle.rs`：complete/stream 入口适配器，共用唯一静态 lifecycle pipeline。
- `lifecycle`：固定阶段顺序、retry/commit、终态和 exactly-once finalize。
- `controllers`：功能规则、状态机、副作用和 typed hook 的唯一 owner。
- `routing`：只保存通用候选账本；previous response 所有权、重放和恢复全部归 `controllers/history.rs`。
- `transport`：账号调用、complete 归一化和 SSE canonical facts；流事件只解码一次后扇出。
- `failure`：通用失败事实与耗尽聚合，不持有功能阈值。
- `errors.rs`：错误分类与客户端错误映射。
- `controllers/telemetry`：唯一拥有调度轨迹、上游诊断和终态遥测构造，再写入 `telemetry::Recorder`。
- `affinity`：会话亲和、账号作用域身份和 Redis 响应归属索引。
- `stream`：流式入口和已提交响应的执行循环；策略与结算归 controller/lifecycle。
...
7. `transport/account` 注入账号 token、`chatgpt-account-id`、账号作用域身份、指纹请求头和 Cookie。
...
`dispatch/lifecycle` 只服务 `v1/*`：非流式与流式共享 request enter 和 stepwise `AttemptRunner`，live stream 使用单次 canonical decode、typed terminal 和唯一 finalizer。功能进入与退出由静态 `ControllerSet` 按固定顺序调用，不存在旧入口包装、兼容 re-export 或新旧双路径。详细契约见 [Responses 请求生命周期控制器设计](request-lifecycle-controller-design.md)。该设计限定在 `dispatch` 内部，不替代鉴权、请求 ID、连接排空等 Tower/Axum 中间件。
...
`cyber_policy` 不属于账号全局故障。提交边界前命中时，attempt 立即排除当前账号并尝试下一个冻结候选；全部候选都不可用时，由通用 attempt outcome 返回最后一个已分类失败，不存在功能私有的重试次数或“三账号”终态。流已经提交时不能透明重放，当前请求保留上游错误并正常结束；对于带显式 `session_id`、`conversation_id` 或 `prompt_cache_key` 的请求，代理按客户端 API Key 隔离、去重记录命中账号，让下一次同会话请求在冻结候选前排除它们。读取过该状态的替代请求成功后，按不可复用 revision token 做 CAS 清除，避免旧成功请求删除并发写入或删除后重建的新失败。状态保留 1 小时，不修改账号状态，也不影响其他会话。状态读取与普通会话亲和查询并行，Redis 操作超过 100 毫秒时 fail-open。携带 `previous_response_id` 的请求不写入该负亲和状态；只有 history controller 已持有可重放完整历史时才允许跨账号，否则保留当前失败。
...
| `cpr:cyber-policy:v2:session:<session_hash>` | API Key 隔离、按账号去重且无功能级数量阈值的会话 `cyber_policy` 排除记录 | 1 小时 |
