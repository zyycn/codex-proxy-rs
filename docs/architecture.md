...
│       ├── accounts_routes/
│       │   ├── query.rs  lifecycle_routes.rs  oauth_routes.rs  quota_routes.rs
...
└── bootstrap/                    # L5 进程装配（原 runtime + config/schema,loader）
    ...
    ├── import_sqlite/
    │   ├── core.rs  telemetry.rs # 导入表组拆分；SQL 仍受 §5.5 唯一豁免约束
    └── tasks/
...
| `upstream/accounts/store/mod.rs` | 1126 | → `store.rs` 门面 + `store/{queries,rows,write}.rs` 按读写与行映射拆分 |
...
新增的 `bootstrap/import_sqlite.rs` 按导入表组拆为 `import_sqlite/{core,telemetry}.rs`，避免一次性迁移命令突破 800 行硬线；它仍是 §5.5 唯一允许跨域写入的入口。
