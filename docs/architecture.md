...
2. 一个功能只有一个规则 owner。阈值、分类、状态机和 decision 只能存在于对应 controller。
...
6. 账号明确失效先在线性化的内存路径生效并驱逐 WebSocket，再异步持久化 PostgreSQL。
...
| quota 与 rate limit | `controllers/quota.rs` |
| 封禁、过期、模型不支持等账号事实 | `controllers/account_failure.rs` |
| 内存失效、异步持久化和 WS 驱逐 effect | `controllers/account_state.rs` |
...
Controller 之间不直接调用。跨功能优先级只在 `ControllerSet` 声明；API、transport、store 和 lifecycle 不重复识别功能语义。
...
2. controller 分类为 quota、rate limit、expired、disabled、banned、model unsupported、Cloudflare 或 cyber policy。
3. 对账号全局事实，账号池 mutex 内立即失效 runtime state 并移除 slot。
4. 驱逐该账号的 idle/busy/connecting WebSocket entry。
5. lifecycle 从冻结候选中选择下一账号。
