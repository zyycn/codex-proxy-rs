...
| OAuth/device/CLI account login/import variants | Original scope includes ChatGPT/Codex OAuth login, device login, CLI token import, manual validated token import, logout/status, quota, and health checks. | Rust now supports validated manual `token`/`refreshToken` import, deterministic admin CLI import from Codex `auth.json`, admin quota fetch, health check, per-account refresh, reset-usage, sanitized admin auth status, and admin logout for imported accounts. Rust still lacks OAuth PKCE login-start/code-relay/callback and device login/poll. | Partial OpenAI/Codex parity. | Continue with OAuth/device login variants without adding proxy pools or non-OpenAI providers. |
...
1. Finish in-scope account operation parity: OAuth PKCE and device login.
...
- [x] Run: `cargo test --test admin_accounts_route_test`
- [ ] Write failing tests for OAuth PKCE login-start/code-relay/callback and device login/poll.
- [ ] Implement remaining OAuth/device scoped operations needed to operate Codex-backed OpenAI GPT routes.
