...
**Goal:** Port the seven OpenAI GPT/Codex parity gaps from the TypeScript reference into the Rust service without adding outbound proxy pools or non-OpenAI/Codex routes.
...
| --- | --- | --- | --- | --- |
| external legacy OpenAI OAuth import / `0b6cc7e` | Account import accepts token/refresh-token account exports and external legacy-shaped OpenAI OAuth data through `AccountImportService`; proxy fields can exist in source data. | Removed. `/admin/accounts/import` now accepts only the native Rust `accounts[]` format and rejects external legacy/proxy/runtime payloads by producing no importable accounts. | Old-project compatibility, out of scope. | Do not restore unless a new product requirement explicitly names external legacy as a first-class import target. |
...
| Account export / `72438c6` | `/auth/accounts/export` supports `full`, `minimal`, `cockpit_tools`, `external`, and `cpa`. | `/admin/accounts/export` supports only the native Rust export format. `full`, `external`, and the other TypeScript export variants return `400`. | Native Rust contract. | Keep only native export while the project has no old deployment or external-format compatibility requirement. |
...
| OAuth/device/CLI account login/import variants | Original scope includes ChatGPT/Codex OAuth login, device login, CLI token import, manual validated token import, logout/status, quota, and health checks. | Rust now supports validated manual `token`/`refreshToken` import, deterministic admin CLI import from Codex `auth.json`, OpenAI OAuth device login/poll, admin-session-gated PKCE login-start/code-relay/callback, admin quota fetch, health check, per-account refresh, reset-usage, sanitized admin auth status, and admin logout for imported accounts. | Scoped OpenAI/Codex parity complete for account-login operations. | Keep admin-session-gated Rust routes and do not add proxy pools or non-OpenAI providers. |
| Proxy/provider metadata and routes | TS project includes per-account proxy assignment, proxy health, and non-OpenAI provider routes. | Rust imports ignore proxy fields and has no outbound proxy/provider pools. | Intentional scope cut. | Keep omitted. Do not add proxy routing, proxy UI/API, non-OpenAI/Codex provider support, or Electron/frontend flows. |
...
3. Keep local client API key utilities documented as Rust-local auth, not TypeScript local client key parity.
...
- [x] Write failing tests for removed external legacy OpenAI OAuth import payloads and native payloads that carry proxy/runtime metadata.
- [x] Run: `cargo test --test admin_accounts_route_test`
- [x] Remove external legacy import normalization and keep `sourceFormat` response marking for the native format.
...
- [x] Run: `cargo test --test admin_accounts_route_test`
- [x] Write failing tests for native account export and removed external legacy/full export formats.
- [x] Implement native account export without external legacy/full format compatibility.
- [x] Run: `cargo test --test admin_accounts_route_test`
...
- [x] Write failing tests for OAuth PKCE login-start/code-relay/callback.
- [x] Implement `/admin/auth/login-start`, `/admin/auth/code-relay`, `/admin/auth/callback`, and the registered `/auth/callback` redirect path with in-memory PKCE sessions and shared validated account import.
- [x] Run: `cargo test --test admin_accounts_route_test admin_auth_`; `cargo test --test oauth_refresh_test`; `cargo test --test admin_accounts_route_test`
...
- [x] Update implementation status with completed parity items and any intentionally omitted non-goals.
