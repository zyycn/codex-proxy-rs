...
- [ ] **Admin account routes:** rebuild `/auth/status`, `/auth/accounts`, import/export, batch delete/status, health-check, per-account refresh, label, delete, reset usage, quota, Cookie CRUD, and quota warnings as `/admin/*` endpoints with admin envelopes and cursor pagination where a list is returned. Encrypted import, manual token add, paginated sanitized list, native/Sub2API export, Sub2API import marking, label/status/delete, batch delete/status, and Cookie CRUD are complete; health/probe, refresh, quota, reset usage, and warnings remain.
...
- [ ] **Client API key management:** implement admin CRUD for local client API keys: list/create/delete/batch-delete/label/status/export/import. Create/list/status/delete/batch-delete/label and `/v1/*` SQLite verification are complete. Remove provider/model binding semantics from the TypeScript version; Rust client API keys authorize only local `/v1/*` access.
...
- [ ] **Account scheduling parity:** extend account acquisition beyond `active + least last_used`. Completed in the in-memory pool: max concurrent slots per account, stale slot cleanup, least-used/round-robin/sticky strategies, quota-cache skip, tier priority, model-plan filtering, exclude IDs, preferred account from session affinity, Cloudflare cooldown skip, request-staggering metadata, capacity summary, settings-backed construction, import-to-pool sync, startup pool snapshots from SQLite, and HTTP fallback acquisition across alternate accounts. Remaining: durable release-time slot state, durable quota-cache restore, and WebSocket/history affinity policy.
- [ ] **Upstream Codex lifecycle:** implement account acquire -> Codex request -> SSE/collect-or-stream -> rate-limit capture -> usage release -> retry/fallback. HTTP SSE collection, streaming passthrough, 401-triggered refresh retry, Codex request context fields, 429/402/403 account-state classification, and fallback across alternate accounts for non-streaming Responses, HTTP SSE setup, and Chat Completions are complete. Remaining: SSE `error`/`response.failed` mapping, successful rate-limit capture, WebSocket/history retry policy, durable 429 quota cooldown persistence, path-block Cookie clearing, and richer OpenAI-compatible error mapping.
- [ ] **Session affinity and previous response:** persist an in-memory response-id to account mapping with TTL, prompt-cache/conversation identity, turn-state replay, and preferred account selection. `src/codex/websocket.rs` now implements the WebSocket `response.create` path for `previous_response_id`; response-id affinity, TTL cleanup, and fallback policy still need to be finished.
...
- [ ] **Admin account management:** Rust has paginated sanitized list, native/Sub2API import, native/Sub2API export, manual token add, label/status/delete, batch delete/status, and Cookie get/set/delete. Missing upstream manual-token validation, per-account refresh/probe, reset usage, quota fetch/update, quota warnings, persistence-health reporting, and JWT metadata/expires extraction during import. SQLite already has `quota_json`, but the repository/pool do not yet restore or update quota windows.
- [ ] **Client API key management:** Rust creates/lists/status-toggles/deletes/batch-deletes/labels local `cpr_` keys and verifies them for `/v1/*`. Missing export and import. Do not port the TypeScript provider/model binding or provider model-fetch APIs.
...
- [ ] **Account scheduling and retry:** Rust has in-memory slots, stale cleanup, strategies, tier priority, preferred/exclude IDs, SQLite startup restore, request-staggering metadata, 429/402/403 fallback across alternate accounts, Cloudflare cooldown skip, and status/token mutation sync for the runtime pool. Missing strict request-interval sleep enforcement, durable quota/cache restore, successful rate-limit header application, Cloudflare path-block tracker, cooldown clearing, WebSocket/history fallback policy, and richer capacity/health endpoints.
