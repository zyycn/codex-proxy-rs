...
- [ ] **Client API key management:** implement admin CRUD for local client API keys: list/create/delete/batch-delete/label/status/export/import. Create/list/status/delete and `/v1/*` SQLite verification are complete. Remove provider/model binding semantics from the TypeScript version; Rust client API keys authorize only local `/v1/*` access.
...
- [ ] **Client API key management:** Rust creates/lists/status-toggles/deletes local `cpr_` keys and verifies them for `/v1/*`. Missing batch-delete, label mutation, export, and import. Do not port the TypeScript provider/model binding or provider model-fetch APIs.
