...
- [x] Add read-only `/admin/settings` for the retained Rust settings surface. The response is admin-session gated, lower camelCase, and excludes proxy URL, Ollama, third-party provider, OpenAI official API key, proxy API key, and Electron/self-update fields. Persistent settings mutation remains a separate task.
...
- [x] Restore the process account pool from SQLite on startup. The snapshot decrypts account tokens through `SecretBox`, carries usage `last_used_at` into scheduling, and lets previously imported accounts survive service restarts.
- [x] Split local client API key auth from admin login. `/admin/api-keys` can create/list local `cpr_` keys, plaintext is returned only once, list responses never expose plaintext/hash, and `/v1/*` verifies enabled keys from SQLite instead of accepting any `cpr_` string.
- [x] Add tested `/v1/responses` streaming passthrough and lifecycle event logs. Client `stream:true` returns `text/event-stream`, keeps the account slot until stream completion or disconnect, parses usage after SSE completion, writes account usage, and records non-secret `v1.response` logs with request/account/route/model/status/latency metadata. Non-streaming calls now write the same lifecycle log after `response.completed`.
- [x] Add tested admin usage-stat read APIs. `/admin/usage-stats` returns cursor-paginated account usage counters, `/admin/usage-stats/summary` returns global totals, and both require the admin session cookie while exposing only non-secret account metadata.
- [x] Add tested upstream-401 refresh failure event logs. When refresh fails, the service records a non-secret `account.refresh` event with request/account/model context, failure classification, and resulting account status.
- [x] Add tested non-streaming output reconstruction. When Codex SSE carries assistant output in `response.output_item.done` or `response.output_text.delta` events but `response.completed.output` is empty, `/v1/responses` now returns a standard assistant `output` message and synchronizes `output_text` for non-streaming clients.
- [x] Add real OpenAI OAuth refresh-token exchange and `/v1/responses` 401 retry wiring. The Rust service refreshes access tokens through the OAuth token endpoint, preserves the existing refresh token when the server omits a replacement, persists the rotated access token, updates the in-process account pool, and retries the failed non-streaming or streaming request once.
...
- [ ] **Admin account routes:** rebuild `/auth/status`, `/auth/accounts`, import/export, batch delete/status, health-check, per-account refresh, label, delete, reset usage, quota, Cookie CRUD, and quota warnings as `/admin/*` endpoints with admin envelopes and cursor pagination where a list is returned. Encrypted import and paginated sanitized list are complete; export, mutations, health, quota, Cookie CRUD, and warnings remain.
...
- [ ] **Client API key management:** implement admin CRUD for local client API keys: list/create/delete/batch-delete/label/status/export/import. Create/list and `/v1/*` SQLite verification are complete. Remove provider/model binding semantics from the TypeScript version; Rust client API keys authorize only local `/v1/*` access.
- [ ] **Settings:** implement admin settings for server/runtime fields that remain in scope: default model, reasoning effort, service tier, model aliases, refresh enabled/margin/concurrency, max concurrent per account, request interval, rotation strategy, tier priority, quota refresh/thresholds/skip exhausted, logs state/capacity/body capture, and usage history retention. Read-only `/admin/settings` is complete; persistent mutation and config-store writeback remain. Remove proxy URL, Ollama, third-party provider, OpenAI official key, and Electron/self-update settings.
...
- [ ] **Logs and usage stats:** add `/admin/logs/state`, clear/detail, error-log equivalents if retained, and richer bounded history queries if needed. Keep log file rotation and SQLite event pagination. Initial `/admin/usage-stats` pagination and `/admin/usage-stats/summary` are complete.
- [ ] **Model catalog:** replace the hard-coded `/v1/models` list with a model store that supports static defaults, configured aliases, `-low/-medium/-high/-xhigh/-fast/-flex` suffix parsing, backend model fetching per plan, cache persistence, `/v1/models/catalog`, `/v1/models/{id}`, `/v1/models/{id}/info`, `/admin/refresh-models`, and `/debug/models`. Static defaults, aliases, suffix parsing, read-only model routes, and debug summary are complete. Backend fetch/cache, plan snapshots, and admin refresh remain.
- [ ] **Account scheduling parity:** extend account acquisition beyond `active + least last_used`. Completed in the in-memory pool: max concurrent slots per account, stale slot cleanup, least-used/round-robin/sticky strategies, quota-cache skip, tier priority, model-plan filtering, exclude IDs, preferred account from session affinity, Cloudflare cooldown skip, request-staggering metadata, capacity summary, settings-backed construction, import-to-pool sync, and startup pool snapshots from SQLite. Remaining: durable release-time slot state and upstream retry/fallback wiring.
- [ ] **Upstream Codex lifecycle:** implement account acquire -> Codex request -> SSE/collect-or-stream -> rate-limit capture -> usage release -> retry/fallback. Initial HTTP SSE collection, streaming passthrough, and 401-triggered refresh retry are complete: `/v1/responses` acquires imported/restored accounts, sends Codex Desktop headers, replays/captures Cookies, collects `response.completed` for non-streaming clients, reconstructs missing assistant output from done items or text deltas, passes through `stream:true` SSE, refreshes and retries once after upstream 401, records refresh failure classification when refresh cannot recover the account, records usage, writes lifecycle logs, and returns the correct OpenAI-compatible JSON or SSE shape. Remaining: full Codex request context fields, SSE `error`/`response.failed` mapping, rate-limit capture, retry/fallback across alternate accounts, 429 quota cache, 402 quota exhausted, 403 ban/Cloudflare distinction, path-block Cookie clearing, and richer OpenAI-compatible error mapping.
...
- [ ] **OpenAI Chat Completions route wiring:** Rust mounts `/v1/chat/completions` to the Responses handler, but that handler parses Responses input and ignores Chat `messages`. `src/translation/openai_to_codex.rs` exists and is unit-tested, but the route is not connected and no ChatCompletion response/SSE translation is wired.
- [ ] **Responses request semantics:** Rust accepts only a minimal Responses body. Missing retained fields from the original include `service_tier` normalization (`fast` -> `priority` upstream), `prompt_cache_key`, `client_metadata`, `include`, `text.format` / tuple-schema reconversion, `tool_choice`, `parallel_tool_calls`, `Version`, `x-codex-turn-metadata`, `x-codex-beta-features`, `x-responsesapi-include-timing-metrics`, `x-codex-parent-thread-id`, `x-codex-window-id`, `session_id`, `x-codex-installation-id`, and sanitized reasoning input behavior.
- [ ] **Responses stream/error parity:** Rust now reconstructs non-stream output from `response.output_item.done` and `response.output_text.delta`, but still ignores SSE `error` and `response.failed` in the non-stream collector, lacks upstream premature-close classification, empty-response retry, stream close `response.failed` injection, tuple reconversion, reasoning replay metadata, function-call metadata, image-generation usage, and detailed OpenAI-compatible error mapping.
- [ ] **Account login flows:** Rust has admin login and account JSON import only. Still missing account-login flows for OAuth PKCE login-start/code-relay/callback, device-code login/poll, Codex CLI auth import, validated manual access-token import, account logout, and `/auth/status`-equivalent admin status. These must be rebuilt as admin/session-gated account-login endpoints, not by treating local client API keys as passwords.
- [ ] **Admin account management:** Rust has paginated sanitized list and import. Missing export, manual add/validation, batch delete, batch status, per-account refresh/probe, label mutation, delete, reset usage, quota fetch/update, Cookie get/set/delete, quota warnings, persistence-health reporting, and JWT metadata/expires extraction during import. SQLite already has `quota_json`, but the repository/pool do not yet restore or update quota windows.
- [ ] **Client API key management:** Rust creates/lists local `cpr_` keys and verifies them for `/v1/*`. Missing delete, batch-delete, label mutation, enable/disable, export, and import. Do not port the TypeScript provider/model binding or provider model-fetch APIs.
- [ ] **Settings:** Rust exposes read-only `/admin/settings` for retained fields. Missing admin-session-gated mutation/writeback with validation for default model, default reasoning effort, service tier, aliases, refresh settings, max concurrent accounts, request interval, rotation strategy, tier priority, quota refresh/threshold/skip settings, log state/capacity/body capture, and usage retention. Do not re-add proxy URL, provider, Ollama, proxy API key, or Electron/self-update settings.
- [ ] **Logs and error diagnostics:** Rust has cursor-paginated SQLite event logs and rotating files. Missing `/admin/logs/state`, pause/enable mutation, clear, detail-by-id, direction/search filters, bounded in-memory request/response body capture, and the original local error-log grouping/raw/count/seen/report/clear APIs if retained for troubleshooting.
- [ ] **Usage and quota background jobs:** Rust records cumulative account usage from `/v1/*`. Missing usage history snapshots (`raw`, `five_min`, `hourly`, `daily`), retention cleanup, passive quota-window recovery, active quota refresher for dirty/limit-reached accounts, quota warning state, and startup background scheduling.
- [ ] **Model catalog:** Rust has a static default catalog, aliases, suffix parsing, read routes, and debug summary. Missing `models.yaml`/cache loading, backend Codex model fetch/probe, per-plan snapshots, model-plan index feeding account scheduling, richer metadata fields, `/admin/refresh-models`, and runtime cache persistence.
- [ ] **Account scheduling and retry:** Rust has in-memory slots, stale cleanup, strategies, tier priority, preferred/exclude IDs, and SQLite startup restore. Missing request-interval staggering enforcement, durable quota/cache restore, 429 `Retry-After`/rate-limit header application, fallback across alternate accounts, Cloudflare path-block tracker, cooldown clearing, WebSocket pool eviction on status/token mutation, and richer capacity/health endpoints.
...
- [ ] **Health and diagnostics:** Rust `/health` only returns `status: ok`. Missing authenticated state, pool summary, capacity summary, transport/fingerprint status, paths, runtime metadata, local-only `/debug/fingerprint`, local-only `/debug/diagnostics`, and upstream test-connection/probe routes.
