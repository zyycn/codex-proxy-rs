...
- [ ] **Admin account routes:** rebuild `/auth/status`, `/auth/accounts`, import/export, batch delete/status, health-check, per-account refresh, label, delete, reset usage, quota, Cookie CRUD, and quota warnings as `/admin/*` endpoints with admin envelopes and cursor pagination where a list is returned. Encrypted import, paginated sanitized list, Sub2API import marking, label/status/delete, batch delete/status, and Cookie CRUD are complete; export, manual add, health/probe, refresh, quota, reset usage, and warnings remain.
...
- [ ] **Admin account management:** Rust has paginated sanitized list, native/Sub2API import, label/status/delete, batch delete/status, and Cookie get/set/delete. Missing export, manual add/validation, per-account refresh/probe, reset usage, quota fetch/update, quota warnings, persistence-health reporting, and JWT metadata/expires extraction during import. SQLite already has `quota_json`, but the repository/pool do not yet restore or update quota windows.
