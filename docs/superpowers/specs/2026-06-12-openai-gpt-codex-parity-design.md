...
Client authentication errors stay OpenAI-compatible. Model misses return `model_not_found`. Upstream 401 triggers refresh and one retry for the same account. 429 and quota exhaustion update account state and retry another account when available. Retryable transport/empty-response failures can retry another account. Terminal upstream errors are formatted as OpenAI-compatible errors for Chat and Responses-compatible failed events for streaming Responses.
