...
3. 只有该 previous ID 能在 Redis 中命中代理管理且可完整重建的 replay 增量链时，才允许清除旧 ID 并透明重放；客户端显式传入但代理没有完整历史时，只在一个账号上原样尝试，失败后保留上游原始 400。

流式预取不能对所有请求统一等待真实输出。应根据请求是否具备安全重放能力选择提交边界：

- 普通新请求：首个可转发事件即可提交，不等待首个真实文本输出。
- 任何 previous_response_id：仅在首个不可逆业务输出或终态前暂存结构帧。managed history 可在此边界内完整重放；external unknown 只用该边界保留原始错误形状，不做账号 fanout。
...
- previous_response_id 命中 `affinity:v2:resp:*` 且增量父链完整：source=Managed，replay_input 为重建后的完整历史。
- previous_response_id 未命中：source=ExternalUnknown，最多在首个调度账号原样尝试，不把未知 ID 广播到其余账号。
- 没有 history：source=None，replay_input=None。

managed history 先尝试所有权账号的原 continuation；失败后在同账号做一次 full replay，账号级失败再按当前设置选择的调度策略继续剩余候选。每个候选都从不可变原请求和同一份重建历史构造新请求。
...
full replay 已经不带 previous ID，但仍保持 `UntilOutputOrTerminal`，这样账号级错误可在真实输出提交前继续剩余候选；不存在固定毫秒 grace window，也不等待首个文本 token 才认定连接有活动。
...
### 14.1 completed 后保存有界增量

每个成功的 `response.completed` 只保存本轮客户端 input、本轮上游 output、父 response ID、depth 和累计字节数。写入前递归删除 `encrypted_content` 和顶层上游对象 ID，保留 `call_id` 等协议关联字段。单节点 2 MiB、单会话 16 MiB、最大 128 层、TTL 4 小时；超限不写 replay，但不修改或截断正常响应。

### 14.2 替换 recover_request_history

删除“无快照也 strip history”的行为。

新函数语义由 `HistoryRecoveryPlan::prepare_attempt` 与 `recover_managed_history` 共同表达：

流程：

1. managed 所有权账号第一次尝试保留 previous ID，并绑定记录的 conversation/turn state。
2. 收到精确 history error、连接丢失或 continuation 不可用时，确认 replay 父链完整。
3. 从增量链重建历史，追加本轮输入，清除 previous ID、turn state 和账号绑定加密内容。
4. 在所有权账号新连接完整重放一次；账号级失败时为当前调度策略给出的下一候选重新构造干净请求。
5. 父链缺失、超限或损坏时返回 false，不把残缺输入伪装成新会话。

只有返回 true 才能 continue 调度循环。

### 14.3 显式 previous ID

客户端显式 previous ID 分两类：

- managed：代理已保存完整增量链，可以按 §14.2 透明恢复和换号。
- external unknown：不删除 ID、不切换账号后发送残缺输入、不广播到账号池；释放 lease 并原样返回上游 status/type/code/message/param/body。

非流式响应：

    HTTP 400
    type = invalid_request_error
    code = previous_response_unavailable  # 仅 managed 链已损坏/超限时
...
managed history 的恢复顺序固定为：

1. 所有权账号携带原 previous ID 直接续接。
2. 精确 history error 或连接不可用时，在所有权账号做一次 full replay。
3. 所有权账号发生账号级失败时，将其加入本请求排除集，再按运行时设置选择的调度策略租用其余候选；每个候选都使用同一份完整历史重建干净请求。

full replay 不携带旧 previous ID、账号绑定 turn state 或 encrypted reasoning。账号轮换必须发挥账号池作用，但不能绕过 Smart、quota-reset-priority、round-robin 或 sticky 等当前配置，也不能把一个节点的污染状态传播给其他节点。
...
- backend/src/dispatch/recovery/history.rs
- backend/src/dispatch/service.rs
- backend/src/dispatch/stream/lifecycle.rs
- backend/src/dispatch/stream/live.rs
- backend/src/dispatch/stream/sse_failure.rs
...
1. managed history + 池连接丢失：所有权账号使用完整 input 恢复，不在新连接发送旧 ID。
2. external unknown previous ID：首个调度账号原样尝试一次；失败后原样返回，不向其余账号广播。
3. managed history 在匹配连接上收到：
...
4. external unknown previous ID 收到同样错误：不重放、不换号。
5. 真实 output_text.delta 后再失败：不重放。
6. function call 参数产生后再失败：不重放。
7. 同一候选的 owner replay 只执行一次，账号级失败后该账号不会再次进入本请求。
8. 恢复先走所有权账号，随后按当前设置选定的调度策略遍历剩余候选。
...
普通日志和 ops error metadata 增加：
    ...
    history_source = none | managed | external_unknown
    ...
    recovery_attempt = 0 | owner_replay | scheduled_candidate
...
| 第一轮完成后主动 discard socket，第二轮为 managed ID | 所有权账号直接发送完整 input，不在新连接发送旧 ID |
| 外部未知 previous ID | 仅首个调度账号原样尝试；失败后保留上游 status/type/code/message/param |
| 所有权账号 full replay 发生账号级失败 | 按当前运行时调度策略尝试剩余候选，且每次都从完整历史重建 |
...
本方案不引入 PostgreSQL schema，但新增 `affinity:v2:*` Redis 逻辑结构并依赖 Redis 持久化维持跨进程 managed replay。回滚只涉及二进制和配置；旧版本不会读取这些键，遗留键按 TTL 自然过期，不增加兼容读取路径。
...
- 显式增量请求没有完整历史时不会伪恢复。
- managed 续接丢失连接时能够用完整输入恢复，并可在账号级失败后调度剩余候选。
- external unknown previous ID 不会广播到账号池。
...
- 本工程只保留 Redis `ResponseReplaySnapshot` 增量链，不再保留进程内 `ImplicitResumeSnapshot` 或第二套 transcript 缓存。
...
这套实现既能解决当前 GPT-5.6 事故，也保持模型无关、低延迟、有界 Redis 持久化和清晰的状态所有权。
