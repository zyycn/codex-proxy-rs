...
本轮 10 个 OpenAI/Codex 链路点已全部复核并写入账本。结论：当前不能声明与原版 100% 一致；“代码证据索引与审计边界”已建立，“OAuth/device/refresh 链路”、“/v1/responses 请求构造与 prompt cache identity”、“HTTP SSE 上游请求链路”、“response/SSE/WS frame 转换链路”、“fallback、retry、错误分类”、“rate-limit、usage、quota、cookie 持久化”和“session affinity、implicit resume、reasoning replay”已对齐；“安全指纹与默认 headers”和“WebSocket 上游链路与连接池”仍为 `部分对齐`。

最高优先级未对齐项：
- 安全模拟仍缺 TLS 指纹证明；WS permessage-deflate offer、协商响应和服务端压缩 frame 解码已补齐测试与实现。
- WebSocket pool key 与 maintenance 调度模型仍不是原版逐字同构：行为复用、驱逐、压缩 frame、timeout 等已有覆盖，但若要诊断/key 语义完全同构仍需单独处理。
...
| 7 | response/SSE/WS frame 转换链路 | 已完成 | 已对齐 |
| 8 | fallback、retry、错误分类 | 已完成 | 已对齐 |
...
- 本轮修正后，streaming `/v1/responses` 在上游请求建立前 fallback exhausted 时，不再返回 OpenAI JSON error 外壳，而是按原版 `streamErrorResponse()`/`PASSTHROUGH_FORMAT.formatStreamError()` 语义返回 HTTP 200 `text/event-stream` 的 `response.failed` SSE；HTTP SSE 401/402/model unsupported/path-block 单账号无 fallback，以及 WebSocket 429/402/model unsupported/path-block fallback exhausted 已由 `tests/codex_serving/upstream_fallback.rs` 和 `tests/codex_serving/responses_websocket.rs` 做结构断言，锁定 `event: response.failed`、动态 `resp_proxy_*` id、`response.error == error`、错误 `type/code` 和 account exhaustion/message 关键语义。这里不再保存每个 mock 组合的 exact stream golden，因为这些是代理生成的下游错误流组合，不是原版或上游提供的返回流样本。
...
- streaming fallback exhausted 外壳已按原版对齐：上游请求建立前的账号级错误在无备用账号时返回 HTTP 200 SSE `response.failed`，而不是 OpenAI JSON error；401 token invalid、402 quota exhausted、model unsupported、Cloudflare path-block 的 HTTP SSE 单账号无 fallback，以及 WebSocket 429/402/model unsupported/path-block fallback exhausted 均已有结构断言覆盖。该类测试只锁定原版稳定语义，不再把 mock account exhaustion 场景组合成 exact golden 返回流。
...
缺口/后续动作：
- 无。原版没有完整 golden fixture 矩阵作为一致性目标。
...
缺口/后续动作：
- 无。当前 transition 已集中 request recovery 与 account fallback acquire/account exhaustion；各入口按自身协议生成最终 `Response`/SSE chunk，这是与原版 Responses passthrough、OpenAI Chat 兼容和 compact JSON 三类外壳对应的边界。
