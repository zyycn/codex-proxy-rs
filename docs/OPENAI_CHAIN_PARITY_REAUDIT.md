...
- dirty quota verification 的状态持久化已补齐第一层：`quota_verify_required` 进入 schema、runtime `Account` 和启动恢复路径；`sqlite_schema_should_persist_quota_verify_required_flag` 与 `account_repository_should_restore_quota_verify_required_into_runtime_pool_accounts` 覆盖该行为。
...
- dirty quota verification 请求前校验尚未对齐。状态字段已经持久化，但 Rust 还未在本地 quota reset 后设置 dirty 标记，也未在下一次请求前最多 5 次调用 `/usage` 验证 dirty 账号。
...
- 继续补 dirty quota verification 运行时机制：窗口离线 reset 后标记需要校验，请求前调用 usage endpoint，失败时保留 dirty 标记并限制放大次数。
