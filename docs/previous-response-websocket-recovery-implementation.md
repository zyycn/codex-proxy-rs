...
- 在正常路径为了优化而把完整请求改写成增量请求；账号 failover 只有持有完整可重建的 replay 链时才允许派生语义等价的完整请求。
...
| 会话污染 | previous ID、turn state、加密 reasoning 与目标账号/socket 不匹配 | 不发送污染值；从完整可重建的 replay 链派生 full replay |
...
风险剖离不得变成请求级熔断。任何账号出现风控信号时，只清理该账号及该连接绑定的 cookie、turn state、previous ID、encrypted reasoning 和 identity；换号仍调用当前设置所选调度策略，不改成固定 Smart、插入顺序、tier 顺序或手工风险队列。只有确定性客户端请求错误、外部未知 history 缺少完整可重建链或输出已提交时才停止跨账号重放。
...
每个 attempt 都从不可变的客户端原始请求或完整重建的 replay input 构造新请求，并重新绑定所选账号的 Authorization、account ID、cookie、Cloudflare 上下文、WebSocket 和冷却状态。前一 attempt 的可变 request、turn state、连接 metadata 或加密 reasoning 不能被下一账号继承。
...
原响应连接中 14 个为 reuse、2 个为 new；后续 211 个失败全部在 new 或 bypass 连接发生。因此账号轮换不是这批生产事故的根因，连接本地缓存丢失才是共同条件。但每个失败请求也都只尝试了一个账号，说明 history error 分支没有发挥账号池；修复不能简单把旧 ID 继续发给下一账号，必须先从完整可重建链生成干净请求。
...
6. 原账号或原 socket 不可用时必须继续账号轮换；有完整可重建的 replay 增量链时清除旧 ID 和账号绑定状态，在本请求剩余的每个候选账号执行 full replay，直至成功或候选耗尽。
...
`KnownConnectionLocal` 丢失原 socket 后，使用完整可重建的增量链在原账号的新连接 full replay；若该账号同时发生账号级故障，再依次重放到其余候选账号。`KnownPersisted` 可以先在符合账号所有权的连接尝试 hydration，失败后同样走 full replay 和完整候选遍历。`ExternalUnknown` 没有代理历史时只能原样尝试；若上游返回 not found，必须原样返回，不能删除 ID 后把不完整 input 当新会话发送，更不能把未知 ID 广播给所有账号。

账号轮换需要一条完整可重建、容量受限的恢复链，而不是在每个响应节点重复保存累计输入：
...
快照作为 `affinity:v2:resp:<response_id>` 值的一部分，与 affinity 同步提交和删除，不写 PostgreSQL，不进入日志。每个 completed response 只保存本轮 input/output 增量和父 response ID；读取时沿父链重建完整业务上下文，Redis 空间随轮数线性增长，不再出现累计 `full_input` 的 O(n²) 放大。单节点 JSON 上限 2 MiB、单会话累计上限 16 MiB、最大 128 层、TTL 4 小时；超限只停止为该响应保存 replay，不截断或修改正常响应。快照不保存 access token、account cookie、上游 account ID、指纹或 Cloudflare 凭据；递归删除 `encrypted_content` 和顶层上游对象 `id`，保留 `call_id`、reasoning summary、tool call/output 等协议关联字段。正常请求仍使用客户端原始 body；只有账号/连接恢复分支读取增量链并派生 full replay。

旧 `ImplicitResumeSnapshot` 与主动 implicit resume 路径已删除。managed history 只有这一条 Redis 增量链恢复模型，不保留第二套临时快照或兼容读取。
...
同 socket 快路径只校验精确 latest response ID；账号所有权与 replay 增量链由 Redis affinity 条目表达，不在连接中复制。需要换 socket 或换账号时，每个候选都通过重建后的 replay input 构造独立派生请求，不修改保留的客户端原始 request，也不复用上一个 attempt 的派生 request。
...
| 可恢复 managed history | connection-local ID 丢失、previous response not found | 从完整可重建的增量链派生 full replay；账号级失败时继续剩余候选 | 成功响应，或候选耗尽后的统一错误 |
...
5. managed previous history 错误在每个候选上只允许一次由完整增量链证明的 full replay；不能复用旧 ID，也不能重复使用上一个账号的派生 request。
...
4. 每个 attempt 从不可变原始请求/重建后的完整业务输入重新构造，并绑定该账号独立的凭据、cookie、风控状态和连接。
5. 连接对象拥有 continuation state，发送前验证 previous ID、owner account、socket 和请求形态；新连接不携带 connection-local ID。
6. Redis 保存唯一的有界 replay 增量链；删除主动 implicit resume 优化及其临时快照，不保留两套恢复模型。
7. history failover 只从校验完整的增量链派生，跨账号剥离旧 ID、turn state 和账号绑定加密内容，业务上下文保持等价。
...
| managed connection-local ID、连接缺失 | 从 Redis 完整增量链重建并重放，不带旧 ID/turn state/账号绑定加密内容 |
...
- 代理管理且未超限的 continuation 都有 Redis 可完整重建的 replay 增量链；节点/会话/深度/TTL 边界明确，链中不含任何账号凭据或 cookie。
...
- node-local token 失效、额度、模型能力、可恢复 5xx/transport、空响应和高风险信号会隔离当前账号并继续遍历当前设置所选调度策略排序后的剩余候选；只有确定性请求错误、外部未知 history 无完整可重建链或输出已提交可以停止跨账号重放。
...
4. 完成后主动 discard，使用 Redis 增量链在原账号新连接完整重放。
...
immutable client request
  ...
  -> bounded replay delta chain for history failover
...
P0/P1 已按上述边界实施：完整候选 ledger 使用运行时设置当前选择的策略顺序（Smart / QuotaResetPriority / RoundRobin / Sticky），没有换号次数/时间预算或风险熔断；managed history 换号只从 Redis 校验完整的有界增量链重放，external unknown history 不广播；WebSocket continuation 由连接拥有，只有 `response.completed` 推进状态。
