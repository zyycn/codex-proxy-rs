...
| --- | --- | --- | --- |
| OpenAI Chat/Responses/Models 兼容 API | `/v1/chat/completions`、`/v1/responses*`、`/v1/models*` | `proxy` | 对外协议合约和请求调度入口。 |
| OpenAI 到 Codex 协议转换 | `core/src/protocol/openai/*`、`core/src/protocol/codex/*` | `proxy/openai`、`codex/protocol` | OpenAI 形状归 `proxy`，Codex 上游形状归 `codex`。 |
| 请求调度、fallback、recovery、implicit resume、reasoning replay | `core/src/serving/*`、`runtime/src/services/dispatch_*` | `proxy/dispatch` | 中转站核心请求链路，不能放进 admin。 |
...
| 管理员登录和 session | `/api/admin/login`、`/api/admin/logout`、`admin_users`、`admin_sessions` | `access`，HTTP 合约在 `admin/session.rs` | 管理端访问控制，不和 client API key 混到 proxy。 |
...
| token refresh 策略、租约和失败状态转换 | `account_refresh_leases`、`auth/oauth.rs` 中 refresh 相关类型 | `accounts/token_refresh/`，任务启动在 `runtime/tasks/token_refresh.rs` | token refresh 是账号生命周期规则，不属于 OAuth 登录文件。 |
...
| Codex fingerprint、installation id、appcast 更新 | `fingerprints`、`fingerprint_update_history`、`fingerprint_update` | `codex/fingerprint.rs`，任务在 `runtime/tasks/fingerprint_update.rs` | fingerprint 是上游身份模拟能力，不是通用配置。 |
| 模型目录和模型快照 | `/v1/models*`、`/api/admin/refresh-models`、`model_plan_snapshots` | `codex/models.rs`，OpenAI 展示在 `proxy/openai/models.rs` | Codex 模型来源归 `codex`，OpenAI 响应格式归 `proxy`。 |
...
| 运行诊断 | `/api/admin/diagnostics`、`/debug/diagnostics`、`/debug/upstream` | `telemetry/diagnostics.rs`，debug route 在 `proxy/openai` | 诊断聚合多个模块状态，但不拥有业务规则。 |
...
| 启动、状态、服务装配、后台任务生命周期 | `server/src/main.rs`、`runtime/src/bootstrap.rs`、`runtime/src/state.rs`、`runtime/src/services.rs`、`runtime/src/tasks/*` | `runtime` | 组合根只接线，不写协议和账号业务。 |
...
| `session_affinities` | `proxy/dispatch/session_affinity.rs` | Responses 会话到账号的亲和性。 |
...
5. HTTP handler 可以集中在 `admin` 或 `proxy`，因为它们是明确的 API 合约层。
6. SQLite store 靠近拥有该数据的模块，不做全局 `repositories` 包。
7. 上游 Codex 链路集中在 `codex`，OpenAI 对外兼容集中在 `proxy`。
8. 统计和事件作为 `telemetry`，因为它同时服务调度、admin dashboard 和审计。
9. `runtime` 只装配依赖和后台任务，不写业务规则。
...
- `src/runtime/services.rs` 是单体进程的组合根，只保留服务实例构造和接线；业务服务和错误类型必须留在 `access/accounts/codex/telemetry/config` 等真实归属模块。
- `admin`、`proxy`、`telemetry` 生产代码不允许直接引用 `crate::runtime::services::*`；handler 只接收 `AppState`，测试构造器可以显式构造 `Services`。
- `AdminLogService`、`AdminLogError`、`AdminLogFilter`、`AdminLogState` 等日志服务类型只归 `telemetry`，`runtime` 只持有构造好的实例。
- `proxy/dispatch/responses.rs` 仍然较大，但 Cloudflare challenge/path-block 识别和恢复是 Chat/Responses 共享规则，已从 Responses 主流程中抽到 `proxy/dispatch/cloudflare.rs`；模型后缀/default reasoning/service tier 的请求选项应用、Codex SSE 完成/失败收集和 replay metadata 提取已下沉到 `codex/protocol/responses.rs`；`codex/transport` 已按 HTTP client、headers、TLS、usage、WebSocket、WebSocket pool 拆开，后续只在职责继续变大时再拆。
...
当前 `rg "crate::runtime::services|runtime::services" src --glob '*.rs'` 的结果只允许出现在 `runtime` 模块自身。
业务模块不得再反向引用组合根。
...
| `proxy/openai/mod.rs` 中的 diagnostics DTO/聚合逻辑 | debug HTTP handler 属于 `proxy/openai/diagnostics.rs`，诊断数据聚合属于 `telemetry/diagnostics.rs`；`mod.rs` 只应声明路由。 | `/debug/diagnostics`、`/debug/fingerprint`、`/debug/upstream` handler 移到 `proxy/openai/diagnostics.rs`，DTO 和聚合只保留在 `telemetry/diagnostics.rs`。 |
| `accounts/admin_service/types.rs` 依赖 `proxy::dispatch::responses` 判断封禁 | 账号导入/补信息不能依赖 proxy 调度层；封禁/停用是 Codex 上游错误语义。 | 封禁 signal 和 `CodexClientError` 分类下沉到 `codex/transport/client.rs`，`accounts` 和 `proxy` 共同引用。 |
| `proxy/dispatch/responses.rs` 中的 `CodexTransport` 决策 | 传输选择只依赖 `CodexResponsesRequest` 字段，不属于 proxy 调度私有规则；Codex protocol、transport 都需要读取。 | `CodexTransport`、`transport_for_request`、`http_sse_fallback_allowed` 移到 `codex/protocol/responses.rs`。 |
...
| `proxy/dispatch/mod.rs` 中重复定义模型目录 client 端口 | 模型目录上游 client 属于 `codex/transport`，proxy dispatch 不应持有重复端口定义。 | 删除 proxy 内重复的 `CodexModelCatalogClient*` 类型，只保留 `codex/transport` 真实出口。 |
...
| `config/mod.rs` 的 `pub use types::*` | 配置类型从根模块星号导出会模糊真实归属，和“直接从真实模块取值”的规则冲突。 | 删除星号导出，调用点直接从 `config/types.rs` 导入。 |
| `admin/mod.rs` 的 response/router/session 再出口 | 管理端内部模块已经有明确文件归属，根模块快捷出口会让调用点看不出是 response、router 还是 session。 | 删除再出口，调用点直接用 `admin::response`、`admin::router`、`admin::session`。 |
...
| `proxy/dispatch/responses.rs` 中的 Cloudflare recovery | Cloudflare challenge/path-block 识别、cookie 清理、cooldown 和 path-block 禁用同时服务 Chat 和 Responses，不属于 Responses 单一路由。 | 移到 `proxy/dispatch/cloudflare.rs`，Chat/Responses 都直接导入真实归属模块，不保留旧路径再出口。 |
| `proxy/dispatch/responses.rs` 中的通用上游错误分类 | 401/402/429、模型不支持、history recovery、retry-after、transport 名称和上游错误体提取同时服务 Chat/Responses，不属于 Responses 主流程；单独 `fallback.rs` 只有状态码布尔判断，属于过渡拆分。 | 共享错误分类和状态码判断收敛到 `proxy/dispatch/upstream_errors.rs`，Chat/Responses 直接依赖共享分类模块。 |
| `proxy/dispatch/responses.rs` 中的 request option 和账号级 upstream 调用 | 模型后缀/default reasoning/service tier 应用属于 Codex Responses 请求语义；transport 映射、quota verify、按账号调用 Codex Responses/Compact 同时服务 Chat/Responses，不属于 Responses 路由主流程；单独 `dispatch/transport.rs` 只有一层映射，属于过渡拆分。 | 请求选项应用移到 `codex/protocol/responses.rs`；Codex 请求到 backend transport 的映射归入 `codex/transport/client.rs`；账号级 upstream 调用移到 `proxy/dispatch/upstream_requests.rs`，同时修正 Chat quota verify 错误文案使用常量值而不是常量名字符串。 |
| `proxy/openai/responses.rs` 中的 Codex SSE 收集和完成 metadata 提取 | 从上游 SSE 判断 `response.completed`/`response.failed`、提取 function call ids 和 replay items 是 Codex Responses 协议语义，dispatch 也需要这些信息做故障转移和亲和性记录。 | `CollectedResponse`、`ResponsesSseFailure`、`response_from_codex_sse`、`completed_response_metadata` 和 tuple schema 回转换下沉到 `codex/protocol/responses.rs`；OpenAI handler 只保留 HTTP DTO、handler、请求转换和 `response.failed` 输出帧编码。 |
| `runtime/tasks/token_refresh.rs` | 原来包含账号 refresh 扫描、定时器、租约、重试、状态转换等业务编排。 | 业务执行器下沉到 `accounts/token_refresh/runtime.rs`；`runtime/tasks/token_refresh.rs` 只保留 ticker、shutdown 和日志包装。 |
| `runtime/tasks/quota_refresh.rs` | 原来包含配额锁定账号扫描、usage 拉取、quota snapshot 持久化、请求错峰和最近刷新窗口。 | 业务执行器下沉到 `accounts/quota/runtime.rs`；`runtime/tasks/quota_refresh.rs` 只保留 ticker、shutdown 和日志包装。 |
...
| `proxy/dispatch/implicit_resume.rs` | 当前文件是 Responses 调度内部的独立规则，职责明确。 | 文档补充该文件。 |
| `proxy/openai/diagnostics.rs` | debug handler 属于 OpenAI/debug API 合约层，和 `telemetry/diagnostics.rs` 的聚合数据职责不同。 | 文档补充该文件。 |
...
### `runtime/services.rs` 收敛规则

`runtime/services.rs` 只能保留：
...
除 `runtime` 自身和测试构造器外，业务模块不允许 import `crate::runtime::services::*`。如果需要共享服务类型，先把类型移动到真实归属模块。
...
| 文件 | 迁入内容 |
| --- | --- |
| `runtime/mod.rs` | 只声明 `bootstrap/state/services/shutdown/tasks`。 |
| `runtime/bootstrap.rs` | 配置加载、路径解析、SQLite 初始化、fingerprint 初始化、router/server 启动入口。 |
| `runtime/state.rs` | `AppState`，持有配置和服务集合；测试构造器也放这里。 |
| `runtime/services.rs` | 构造 `access/accounts/codex/proxy/telemetry/admin` 需要的服务实例。 |
| `runtime/shutdown.rs` | signal、graceful shutdown、后台任务停止信号。 |
| `runtime/tasks/coordinator.rs` | 根据配置启动/停止所有后台任务。 |
| `runtime/tasks/cookie_cleanup.rs` | 调用 `accounts/cookies.rs` 清理过期 Cookie。 |
| `runtime/tasks/fingerprint_update.rs` | 调用 `codex/fingerprint.rs` 检查 appcast 并写入更新历史。 |
| `runtime/tasks/model_refresh.rs` | 调用 `codex/models.rs` 周期刷新模型目录。 |
| `runtime/tasks/quota_refresh.rs` | 启动 quota refresh ticker、处理 shutdown、调用 `accounts/quota/runtime.rs`。 |
| `runtime/tasks/session_affinity_cleanup.rs` | 调用 `proxy/dispatch/session_affinity.rs` 清理过期亲和性。 |
| `runtime/tasks/session_cleanup.rs` | 调用 `access/admin_session.rs` 清理过期管理员 session。 |
| `runtime/tasks/token_refresh.rs` | 启动 token refresh ticker、处理 shutdown、调用 `accounts/token_refresh/runtime.rs`。 |
...
| `http/router.rs` | merge `proxy`、`admin`、`web` 三组路由。 |
...
| 文件 | 迁入内容 |
| --- | --- |
| `proxy/mod.rs` | 导出 OpenAI 兼容层、鉴权接线、dispatch。 |
| `proxy/auth.rs` | 从 HTTP header 提取 client API key 并调用 `access` 校验。 |
| `proxy/openai/chat.rs` | Chat Completions DTO、handler、到 Codex Responses 的转换入口。 |
| `proxy/openai/diagnostics.rs` | `/debug/diagnostics`、`/debug/fingerprint`、`/debug/upstream` debug handler。 |
| `proxy/openai/errors.rs` | OpenAI 兼容错误响应、上游错误映射。 |
| `proxy/openai/models.rs` | `/v1/models*` DTO、handler、OpenAI 模型展示格式。 |
| `proxy/openai/responses.rs` | Responses DTO、handler、review/compact 变体入口、OpenAI `response.failed` SSE 输出帧编码。 |
| `proxy/openai/sse.rs` | OpenAI SSE 输出编码、stream 错误帧。 |
| `proxy/dispatch/chat.rs` | Chat Completions 调度服务。 |
| `proxy/dispatch/cloudflare.rs` | Cloudflare challenge/path-block 识别、Cookie 清理、账号 cooldown 和 path-block 禁用恢复。 |
| `proxy/dispatch/responses.rs` | Responses HTTP SSE/WebSocket 调度主流程、implicit resume。 |
| `proxy/dispatch/implicit_resume.rs` | implicit resume 条件判断、history strip 和 previous response 缺失恢复辅助。 |
| `proxy/dispatch/reasoning_replay.rs` | reasoning replay cache 和 replay 条件。 |
| `proxy/dispatch/session_affinity.rs` | conversation/session affinity 计算、存储、清理。 |
| `proxy/dispatch/upstream_errors.rs` | Chat/Responses 共享的上游错误分类、fallback 状态码判断、retry-after 冷却、transport 名称和错误体提取。 |
| `proxy/dispatch/upstream_requests.rs` | Chat/Responses 共享的 quota verify、账号级 Codex Responses/Compact 调用和单账号 5xx retry。 |
...
| 模块 | 职责 | 不放什么 |
| --- | --- | --- |
| `runtime` | 组合根、启动、状态、服务装配、后台任务生命周期。 | 协议转换、SQL 查询、HTTP DTO。 |
...
| `proxy` | OpenAI 兼容 API、协议转换、client key 鉴权接线、请求调度、fallback/retry/reasoning replay/session affinity。 | admin 前端 DTO、SQLite schema 初始化。 |
...
OpenAI 兼容接口、协议转换和调度是同一条用户请求链路。把它们放进 `proxy` 可以让一次
Responses 行为修改集中在：

```text
proxy/openai/responses.rs
proxy/dispatch/responses.rs
...
账号池是中转站运行核心，不只是前端管理页面。它同时被 proxy dispatch、token refresh、
...
| 当前位置 | 目标位置 |
| --- | --- |
| `crates/server/src/main.rs` | `src/main.rs`、`src/runtime/bootstrap.rs`、`src/runtime/shutdown.rs` |
| `crates/runtime/src/bootstrap.rs` | `src/runtime/bootstrap.rs` |
| `crates/runtime/src/state.rs` | `src/runtime/state.rs` |
| `crates/runtime/src/services.rs` | `src/runtime/services.rs` |
| `crates/runtime/src/repositories.rs` | 删除，按数据归属移动到各模块 store 初始化。 |
| `crates/runtime/src/tasks/*` | `src/runtime/tasks/*` |
...
| `crates/core/src/gateway/conversation.rs` | `src/proxy/dispatch/session_affinity.rs` |
...
| 当前位置 | 目标位置 |
| --- | --- |
| `crates/core/src/protocol/openai/chat.rs` | `src/proxy/openai/chat.rs` |
| `crates/core/src/protocol/openai/responses.rs` | `src/proxy/openai/responses.rs` |
| `crates/core/src/protocol/openai/models.rs` | `src/proxy/openai/models.rs` |
| `crates/server/src/openai_api/auth.rs` | `src/proxy/auth.rs` |
| `crates/server/src/openai_api/error.rs` | `src/proxy/openai/errors.rs` |
| `crates/server/src/openai_api/sse.rs` | `src/proxy/openai/sse.rs` |
| `crates/server/src/openai_api/chat.rs` | `src/proxy/openai/chat.rs` |
| `crates/server/src/openai_api/responses.rs` | `src/proxy/openai/responses.rs` |
| `crates/server/src/openai_api/models.rs` | `src/proxy/openai/models.rs` |
| `crates/server/src/openai_api/diagnostics.rs` | 诊断聚合在 `src/telemetry/diagnostics.rs`，debug handler 在 `src/proxy/openai/diagnostics.rs`。 |
| `crates/server/src/openai_api/router.rs` | `src/proxy/openai/mod.rs` |
| `crates/core/src/serving/fallback.rs` | 状态码级 fallback 判断并入 `src/proxy/dispatch/upstream_errors.rs`。 |
| `crates/core/src/serving/recovery.rs` | Cloudflare 识别和账号恢复在 `src/proxy/dispatch/cloudflare.rs`，上游错误分类和通用 fallback 状态判断在 `src/proxy/dispatch/upstream_errors.rs`。 |
| `crates/core/src/serving/reasoning_replay.rs` | `src/proxy/dispatch/reasoning_replay.rs` |
| `crates/core/src/serving/implicit_resume.rs` | `src/proxy/dispatch/implicit_resume.rs` |
| `crates/core/src/serving/responses.rs` | 主流程在 `src/proxy/dispatch/responses.rs`，Codex 请求选项和 SSE 收集在 `src/codex/protocol/responses.rs`，backend transport 映射在 `src/codex/transport/client.rs`，账号级 upstream 调用在 `src/proxy/dispatch/upstream_requests.rs`。 |
| `crates/core/src/serving/affinity.rs` | `src/proxy/dispatch/session_affinity.rs` |
| `crates/adapters/src/sqlite/session_affinity.rs` | `src/proxy/dispatch/session_affinity.rs` |
| `crates/runtime/src/services/dispatch_chat.rs` | `src/proxy/dispatch/chat.rs` |
| `crates/runtime/src/services/dispatch_responses.rs` | `src/proxy/dispatch/responses.rs` |
...
```text
runtime -> all modules
http -> admin, proxy, web
admin -> access, accounts, codex, telemetry, config
proxy -> access, accounts, codex, telemetry
...
- `codex` 不能依赖 `proxy` 或 `admin`。
- `accounts` 不能依赖 `admin`。
- `telemetry` 不能依赖 `admin`。
- `infra` 不能依赖任何业务模块。
- `config` 不能依赖任何业务模块。
- `proxy/openai` 不能直接写 SQLite，只调用 service/store 抽象或具体 service。
...
publish = false
rust-version = "1.95"
description = "Codex/OpenAI proxy backed by ChatGPT accounts."
...
3. 每个业务目录用自己的 `mod.rs` 声明子测试文件，例如 `tests/integration/proxy/mod.rs`。
...
| 改 OpenAI Chat/Responses/Models 兼容格式 | `proxy/openai` |
| 改 dispatch、fallback、retry、reasoning replay、session affinity | `proxy/dispatch` |
