...
async fn unified_login_returns_server_identity_and_rotates_the_previous_session() {
    ...
    let key = login(&app, json!({"type": "key", "apiKey": RAW_KEY}), None).await;
    assert_eq!(key.status(), StatusCode::OK);
    let key_cookie = session_cookie(&key);
    assert_eq!(response_json(key).await["data"]["type"], "key");
    ...
    assert_eq!(admin_data["type"], "admin");
...
async fn valid_key_session_cannot_read_or_mutate_admin_resources() {
    let (app, _) = client_app().await;
    let cookie =
        session_cookie(&login(&app, json!({"type": "key", "apiKey": RAW_KEY}), None).await);
    for path in [
...
async fn admin_session_is_not_a_key_session() {
    ...
    assert_eq!(
        response_json(get(&app, "/api/auth/status", &cookie).await).await["data"]["session"]["type"],
        "admin"
    );
}
...
async fn declared_roles_and_foreign_key_ids_cannot_override_server_authority() {
    ...
    let cookie =
        session_cookie(&login(&app, json!({"type": "key", "apiKey": RAW_KEY}), None).await);
    for body in [
        json!({"type": "admin", "apiKey": RAW_KEY}),
        json!({"type": "key", "apiKey": RAW_KEY, "adminUserId": "admin_1"}),
        json!({"type": "key", "apiKey": RAW_KEY, "clientKeyId": "other-key"}),
        json!({"type": "superadmin", "apiKey": RAW_KEY}),
        json!({"apiKey": RAW_KEY}),
    ] {
    ...
    let invalid = login(
        &app,
        json!({"type": "admin", "password": RAW_KEY}),
        Some(&cookie),
    )
    .await;
...
async fn disabled_key_revokes_its_session_and_reenabling_does_not_restore_it() {
    ...
    assert_eq!(
        login(&app, json!({"type": "key", "apiKey": RAW_KEY}), None)
            .await
            .status(),
        StatusCode::UNAUTHORIZED
    );
