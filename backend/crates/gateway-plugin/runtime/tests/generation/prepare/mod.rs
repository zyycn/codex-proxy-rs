...
async fn registration_with_a_changed_contribution_id_is_rejected() {
    ...
    let candidate =
        PluginPreparation::prepare(&runtime, snapshot.config_revision, snapshot.clone())
            .await
    ...
    let error = PluginPreparation::prepare(&runtime, snapshot.config_revision, snapshot)
        .await
...
async fn diagnostics_distinguish_running_failed_preparation_and_draining_generations() {
    ...
    let active = PluginPreparation::prepare(&runtime, snapshot.config_revision, snapshot.clone())
        .await
        .unwrap();
    ...
    assert!(
        PluginPreparation::prepare(&runtime, snapshot.config_revision, failed.clone())
            .await
            .is_err()
    );
...
async fn preparing_diagnostics_never_wait_for_the_serialized_prepare_io() {
    ...
    let mut preparation = Box::pin(PluginPreparation::prepare(
        &runtime,
...
async fn restart_circuit_caps_short_lived_crashes_and_a_new_revision_recovers() {
    ...
    let disabled_generation =
        PluginPreparation::prepare(&runtime, Revision::new(1).unwrap(), disabled.clone())
            .await
    ...
    let replacement = PluginPreparation::prepare(&runtime, Revision::new(1).unwrap(), repaired)
        .await
...
async fn restart_circuit_ignores_planned_generation_shutdown() {
    ...
    let first = PluginPreparation::prepare(&runtime, snapshot.config_revision, snapshot.clone())
        .await
    ...
    let second = PluginPreparation::prepare(&runtime, snapshot.config_revision, snapshot)
        .await
...
async fn failed_candidate_leaves_the_previous_generation_ready() {
    ...
    assert!(
        PluginPreparation::prepare(&runtime, candidate.config_revision, candidate)
            .await
            .is_err()
    );
    assert!(active.is_ready());
...
async fn disabled_missing_artifacts_do_not_block_the_remaining_generation_or_offline_recovery() {
    ...
    let active = PluginPreparation::prepare(&runtime, snapshot.config_revision, snapshot)
        .await
        .unwrap();
    assert!(active.is_ready());
    drop(active);
