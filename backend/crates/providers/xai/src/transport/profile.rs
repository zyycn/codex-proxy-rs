...
impl GrokCliReleaseService {
    ...
    #[must_use]
    pub(crate) fn status(&self) -> GrokCliReleaseStatus {
        self.status.clone()
    }
...
pub(crate) struct GrokCliReleaseSnapshot {
...
pub(crate) struct GrokCliReleaseStatus {
    snapshot: Arc<RwLock<GrokCliReleaseSnapshot>>,
}

impl GrokCliReleaseStatus {
    #[must_use]
    pub(crate) fn snapshot(&self) -> GrokCliReleaseSnapshot {
        self.snapshot
            .read()
            .unwrap_or_else(std::sync::PoisonError::into_inner)
            .clone()
