...
async fn socks_account_egress_preserves_dns_mode_auth_and_ipv6_for_sse_and_websocket() {
    for websocket in [false, true] {
        let mut targets = vec![("socks5", "localhost"), ("socks5h", "upstream.invalid")];
        // The reqwest SOCKS connector currently encodes bracketed IPv6 as a domain.
        if websocket {
            targets.push(("socks5h", "[::1]"));
        }
