...
pub enum RegistryError {
    #[error("provider extension generation is unavailable")]
    GenerationUnavailable,
    #[error("provider extension generation is already registered")]
    DuplicateGeneration,
    /// Provider 名称无效。
...
/// 不可变 Provider 视图；动态入口按请求冻结的集合解析。
#[derive(Clone, Default)]
pub struct ProviderRegistry {
    pub(super) providers: Arc<BTreeMap<ProviderKind, Arc<dyn Provider>>>,
    pub(super) extensions: Option<super::extensions::ProviderExtensionIndex>,
    pub(super) lease: Option<crate::runtime::extensions::ExtensionSetReference>,
}

impl ProviderRegistry {
    pub fn for_extensions(
        &self,
        reference: Option<&crate::runtime::extensions::ExtensionSetReference>,
    ) -> Result<Self, RegistryError> {
        let Some(index) = &self.extensions else {
        ...
        let mut registry = index
            ...
            .ok_or(RegistryError::GenerationUnavailable)?
            .as_ref()
            .clone();
        registry.lease = Some(reference.clone());
        Ok(registry)
    }
...
impl ProviderCatalogPort for ProviderRegistry {
    fn for_extensions(
        &self,
        reference: Option<&crate::runtime::extensions::ExtensionSetReference>,
    ) -> Result<Option<Arc<dyn ProviderCatalogPort>>, ProviderCatalogUnavailable> {
        self.for_extensions(reference)
            .map(|registry| Some(Arc::new(registry) as Arc<dyn ProviderCatalogPort>))
