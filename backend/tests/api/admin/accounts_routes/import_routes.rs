...
async fn admin_accounts_import_should_store_cpr_account_tokens() {
    let (pool, _dir) = init_test_db("admin-accounts-import").await;
    let redis = create_test_redis("admin-accounts-import").await;
...
async fn admin_accounts_import_should_read_sub2api_backup_access_token_only_accounts() {
    ...
    Mock::given(method("GET"))
        ...
        .await;
    let (app, _state, pool, _dir) = admin_accounts_test_app_with_api_base_url(
        "admin-accounts-import-sub2api-backup-at",
        127,
        format!("{}/backend-api", server.uri()),
    )
    .await;
...
async fn admin_accounts_import_should_read_cliproxyapi_codex_auth_files() {
    ...
    Mock::given(method("GET"))
        ...
        .await;
    let (app, _state, pool, _dir) = admin_accounts_test_app_with_api_base_url(
        "admin-accounts-import-cliproxyapi",
        128,
        format!("{}/backend-api", server.uri()),
    )
    .await;
...
async fn admin_accounts_import_should_fetch_wham_usage_for_current_openai_token_identity() {
    ...
    Mock::given(method("GET"))
        ...
        .await;
    let (app, _state, pool, _dir) = admin_accounts_test_app_with_api_base_url(
        "admin-accounts-import-current-openai-token",
        118,
        format!("{}/backend-api", server.uri()),
    )
    .await;
...
async fn admin_accounts_import_should_complete_chatgpt_account_id_from_refresh_token() {
    ...
    let (app, _state, pool, _dir) =
        admin_accounts_test_app_with_api_base_url_and_oauth_token_endpoint(
            "admin-accounts-import-rt-complete",
            125,
            format!("{}/backend-api", server.uri()),
...
async fn admin_accounts_import_from_refresh_token_should_not_store_consumed_refresh_token_when_not_rotated()
 {
    ...
    let (app, _state, pool, _dir) = admin_accounts_test_app_with_oauth_token_endpoint(
        "admin-accounts-import-rt-no-rotation",
        126,
        format!("{}/oauth/token", server.uri()),
    )
    .await;
...
async fn admin_accounts_import_should_fetch_usage_to_complete_missing_plan_and_quota() {
    ...
    let (pool, _dir) = init_test_db("admin-accounts-import-supplement").await;
    let redis = create_test_redis("admin-accounts-import-supplement").await;
    seed_admin_session(&pool, &redis, "session_1").await;
    ...
    let status = response.status();
    let body = response_json(response).await;
    let stored = PgAccountStore::new(pool.clone())
        .get("acct_import_supplement")
        .await
        .unwrap()
        .unwrap();
    let quota_json: Option<Value> =
        sqlx::query_scalar("select quota_json from accounts where id = $1")
            .bind("acct_import_supplement")
            .fetch_one(&pool)
            .await
            .unwrap();
    let quota = quota_json.unwrap();
...
async fn admin_accounts_import_should_update_existing_sub2api_account_and_clear_stale_quota_lock() {
    let (app, _state, pool, _dir) =
        admin_accounts_test_app("admin-accounts-import-update-existing", 116).await;
    ...
    .await;
    sqlx::query("update accounts set quota_limit_reached = true, quota_cooldown_until = $1, quota_verify_required = true where id = $2")
        .bind(crate::support::storage::timestamp("2026-06-25T00:00:00Z")).bind("acct_import_update").execute(&pool).await.unwrap();
    ...
    let quota_state: (
        ...
        "select quota_limit_reached, quota_cooldown_until, quota_verify_required, quota_json from accounts where id = $1",
...
async fn admin_accounts_import_should_require_admin_session_cookie() {
    let (pool, _dir) = init_test_db("admin-accounts-import-auth").await;
    let redis = create_test_redis("admin-accounts-import-auth").await;
    let config = test_config(test_database_url());
    let stores = background_task_stores(pool.clone(), redis);
    let profile = crate::support::wire_profile::test_wire_profile_value();
    let services = Arc::new(Services::new(&config, stores, wire_profile(profile)));
...
async fn admin_accounts_import_should_store_plain_tokens_and_list_sanitized_accounts() {
    let (app, _state, pool, _dir) =
        admin_accounts_test_app("admin-accounts-import-sanitized", 113).await;
    let access_token = test_jwt(
        "chatgpt-account",
        Some("chatgpt-user"),
