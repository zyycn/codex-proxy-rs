...
async fn admin_accounts_export_should_reject_empty_ids() {
    let (app, _state, pool, _dir) = admin_accounts_test_app("admin-accounts-export-all", 131).await;
    seed_account(
        &pool,
        NewAccount {
...
async fn admin_accounts_export_should_filter_by_ids() {
    let (app, _state, pool, _dir) =
        admin_accounts_test_app("admin-accounts-export-filter", 132).await;
    seed_account(
        &pool,
        NewAccount {
...
async fn admin_accounts_export_should_fail_when_selected_id_is_missing() {
    let (app, _state, _pool, _dir) =
        admin_accounts_test_app("admin-accounts-export-missing", 133).await;
...
async fn admin_accounts_export_payload_should_import_directly() {
    let (source_app, _source_state, source_pool, _source_dir) =
        admin_accounts_test_app("admin-accounts-export-source", 134).await;
    seed_account(
        &source_pool,
        NewAccount {
    ...
    let (target_app, _target_state, target_pool, _target_dir) =
        admin_accounts_test_app("admin-accounts-export-target", 135).await;
    ...
    let status = response.status();
    let body = response_json(response).await;
    let stored = PgAccountStore::new(target_pool)
...
async fn admin_accounts_export_should_require_explicit_sensitive_export_confirmation() {
    let (app, _state, pool, _dir) =
        admin_accounts_test_app("admin-accounts-export-confirm", 136).await;
    seed_account(
        &pool,
        NewAccount {
...
async fn admin_accounts_export_should_not_write_usage_record_on_success() {
    let (app, _state, pool, _dir) =
        admin_accounts_test_app("admin-accounts-export-usage", 137).await;
    seed_account(
        &pool,
        NewAccount {
