...
use codex_proxy_rs::accounts::quota::{
...
use codex_proxy_rs::accounts::store::{NewAccount, PgAccountStore};
...
async fn quota_refresh_service_should_send_usage_cookie_when_cookie_store_is_configured() {
    ...
    let service = RuntimeQuotaRefreshService::new(
        store,
...
async fn quota_refresh_service_should_skip_recent_locked_account_before_cooldown_expires() {
    ...
    let service = RuntimeQuotaRefreshService::with_min_refresh_interval_secs(
        store,
        PgAccountUsageStore::new(pool.clone()),
        Arc::new(codex),
        1_800,
