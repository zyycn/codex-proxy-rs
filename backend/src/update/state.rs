...
pub(crate) fn read_state(path: &Path) -> Result<SystemUpdateStatusData, UpdateError> {
    if !path.exists() {
...
fn write_state(path: &Path, state: &SystemUpdateStatusData) -> Result<(), UpdateError> {
...
impl OperationLock {
    pub(crate) fn acquire(path: &Path) -> Result<Self, UpdateError> {
        if let Some(parent) = path
            .parent()
            .filter(|parent| !parent.as_os_str().is_empty())
        {
        ...
        match Self::try_create(path) {
            ...
            Err(error) if error.kind() == io::ErrorKind::AlreadyExists => {
                ...
                Err(UpdateError::conflict("System update already running"))
            }
...
fn stale_lock(path: &Path) -> Result<bool, UpdateError> {
...
fn internal_error(context: &str, error: impl std::fmt::Display) -> UpdateError {
