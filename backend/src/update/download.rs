...
pub(super) async fn download_file(
    ...
    if !response.status().is_success() {
        return Err(UpdateError::bad_gateway(format!(
        ...
        .map_err(bad_gateway_with("Download stream failed"))?
    {
        ...
        if downloaded > max_size {
            return Err(UpdateError::bad_request(
                "Download exceeds max allowed size",
            ));
        }
...
pub(super) async fn verify_checksum(
    ...
    let expected = expected.ok_or_else(|| UpdateError::bad_gateway("Checksum not found"))?;
    let actual = sha256_file(file_path)?;
    if expected.eq_ignore_ascii_case(&actual) {
        Ok(())
    } else {
        Err(UpdateError::bad_gateway("Checksum mismatch"))
    }
}
...
pub(super) fn validate_download_url(
    ...
    let host = url
        .host_str()
        .ok_or_else(|| UpdateError::bad_request("Download URL is missing host"))?;
    ...
    if allowed {
        ...
        Err(UpdateError::bad_request(format!(
