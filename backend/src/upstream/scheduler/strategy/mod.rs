...
use crate::upstream::scheduler::feedback::FeedbackStats;
...
pub fn select(
    ...
    match strategy {
        RotationStrategy::Smart => smart::select(input, cursor),
        RotationStrategy::QuotaResetPriority => quota_reset::select(input, cursor),
        RotationStrategy::RoundRobin => round_robin::select(input, cursor),
        RotationStrategy::Sticky => sticky::select(input, cursor),
    }
}
