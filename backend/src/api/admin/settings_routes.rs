...
impl AdminSettingsData {
    fn from_settings(settings: &SettingsSnapshot) -> Self {
        Self {
...
fn parse_settings_patch(body: &[u8]) -> Result<SettingsPatch, AdminError> {
...
fn settings_service_error(error: SettingsError) -> AdminError {
    match error {
        SettingsError::InvalidField(SettingsValidationError::InvalidField { field, message }) => {
            AdminError::bad_request(format!("Invalid setting {field}: {message}"))
        }
        SettingsError::Database(_) | SettingsError::Json(_) | SettingsError::StoredField { .. } => {
