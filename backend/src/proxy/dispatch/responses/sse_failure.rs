...
pub(super) const STREAM_DISCONNECTED_CODE: &str = "stream_disconnected";
pub(super) const STREAM_DISCONNECTED_MESSAGE: &str =
    "Upstream stream closed before response.completed";

pub(super) fn sse_failure_error_body(failure: &ResponsesSseFailure) -> String {
...
pub(super) fn is_model_unsupported_sse_failure(failure: &ResponsesSseFailure) -> bool {
    sse_failure_matches(failure, is_model_unsupported_signal)
}

pub(super) fn is_history_recovery_sse_failure(failure: &ResponsesSseFailure) -> bool {
    sse_failure_matches(failure, is_history_recovery_signal)
}

pub(super) fn sse_failure_invalid_reasoning_replay(failure: &ResponsesSseFailure) -> bool {
    sse_failure_matches(failure, is_invalid_encrypted_content_signal)
}

pub(super) fn client_error_invalid_reasoning_replay(error: &CodexClientError) -> bool {
    matches!(
        error,
...
pub(super) fn first_sse_failure(
    prefetched: &[u8],
...
pub(super) fn stream_failure_metadata(
    failure: &ResponsesSseFailure,
...
pub(super) fn stream_failure_source(failure: &ResponsesSseFailure) -> &'static str {
...
pub(super) fn synthetic_stream_disconnected_detail(
    failure: &ResponsesSseFailure,
) -> Option<String> {
...
pub(super) fn status_code_for_stream_failure(failure: &ResponsesSseFailure) -> i64 {
    let code = failure
        .upstream_code
        .as_deref()
        .unwrap_or("error")
...
pub(super) fn stream_failure_http_status(failure: &ResponsesSseFailure) -> u16 {
