...
mod tests {
    ...
    struct FirstWriteBlocking {
        started_tx: mpsc::SyncSender<()>,
        release_rx: Option<mpsc::Receiver<()>>,
    }

    impl Write for FirstWriteBlocking {
    ...
    fn tracing_config_should_reject_disabled_outputs() {
        let config = TracingConfig::new("info", false, None);

        assert!(config.validate().is_err());
    }

    #[test]
    fn tracing_config_should_accept_each_output_independently() {
        let directory = tempfile::tempdir().unwrap();
        let stdout_only = TracingConfig::new("info", true, None);
        let file_only = TracingConfig::new(
            "info",
            false,
            Some(RotationConfig::new(directory.path(), 14, 1024, 20)),
        );

        assert_eq!(
            (stdout_only.validate().is_ok(), file_only.validate().is_ok()),
            (true, true)
        );
    }

    #[test]
    fn dropped_log_report_should_emit_emergency_json_for_full_buffer() {
        let (started_tx, started_rx) = mpsc::sync_channel(0);
        let (release_tx, release_rx) = mpsc::sync_channel(0);
        let (mut writer, guard) = NonBlockingBuilder::default()
            .lossy(true)
            .buffered_lines_limit(1)
            .finish(FirstWriteBlocking {
                started_tx,
        ...
        report_dropped_logs(&mut [NamedErrorCounter::new("test", counter)], &mut output);
        release_tx.send(()).unwrap();
        drop(writer);
        drop(guard);

        let warning: serde_json::Value = serde_json::from_slice(&output).unwrap();
        assert_eq!(
            (
                warning["level"].as_str(),
                warning["fields"]["output"].as_str(),
                warning["fields"]["dropped"].as_u64(),
            ),
            (Some("ERROR"), Some("test"), Some(1))
        );
    }
}
