...
impl AccountManageService {
    pub async fn import(
        ...
        let parsed = crate::fleet::import::parse_account_import_payload(&data)
    ...
    async fn import_entry(
        &self,
        entry: crate::fleet::import::AccountImportEntry,
        source: crate::fleet::import::AccountImportSource,
        ...
        let label = crate::fleet::import::normalize_label(entry.label);
        if label
            .as_ref()
        ...
        let access_token_expires_at = entry
            ...
            .as_deref()
            .map(crate::fleet::import::parse_account_import_datetime)
            .transpose()
        ...
        let mut plan_type = claims
            ...
            .or_else(|| crate::fleet::import::normalize_nonempty(entry.plan_type));
        ...
        let email = claims
            ...
            .or_else(|| crate::fleet::import::normalize_nonempty(entry.email.clone()));
        let chatgpt_account_id = claims
            .and_then(|claims| claims.account_id.as_deref())
            .or_else(|| crate::fleet::import::normalize_nonempty_str(entry.account_id.as_deref()));
        let chatgpt_user_id = claims
            .and_then(|claims| claims.user_id.as_deref())
            .or_else(|| crate::fleet::import::normalize_nonempty_str(entry.user_id.as_deref()));
        ...
        let account = crate::fleet::store::NewAccount {
            id: account_id.clone(),
            email,
    ...
    async fn resolve_import_tokens(
        ...
        let refresh_token = crate::fleet::import::normalize_nonempty(refresh_token);
        let Some(access_token) = crate::fleet::import::normalize_nonempty(
            token
                .as_deref()
                .map(crate::fleet::import::normalize_bearer_token),
        ) else {
        ...
        if let Ok(claims) = crate::fleet::refresh::manual_account_claims(&access_token, Utc::now())
        {
            return Ok(Some(ResolvedImportTokens {
                access_token,
                refresh_token,
    ...
    async fn import_target_account_id(
        ...
        let provided_id = crate::fleet::import::normalize_nonempty_str(id).map(ToString::to_string);
