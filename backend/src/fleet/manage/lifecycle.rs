...
impl AccountManageService {
    ...
    pub async fn create(
        ...
        let provided_refresh_token = crate::fleet::import::normalize_nonempty(refresh_token);
        let tokens = if let Some(access_token) = crate::fleet::import::normalize_nonempty(
            token
                .as_deref()
        ...
        let account_id = if let Some(existing) = existing {
            let updated = self
                ...
                .update_from_claims(
                    &existing.id,
                    crate::fleet::store::AccountClaimsUpdate {
                        ...
                        status: crate::fleet::account::AccountStatus::Active,
                    },
                )
                .await
            ...
            let id = crate::fleet::import::normalized_account_id(None);
            self.store
                .insert(crate::fleet::store::NewAccount {
                    id: id.clone(),
                    email: claims.email.clone(),
                    account_id: claims.account_id.clone(),
                    user_id: claims.user_id.clone(),
    ...
    pub async fn update_account(
        ...
        let updated = self
            ...
            .update_metadata(
                account_id,
                crate::fleet::store::AccountMetadataUpdate {
                    email: None,
                    account_id: None,
                    user_id: None,
                    label: update.label,
        ...
        match refresh_result {
            ...
            Err(AccountManageError::RefreshTokenExchange(failure)) => {
                if manual_refresh_failure_is_permanent(&failure) {
                    let status = crate::fleet::account::AccountStatus::Expired;
                    let updated = self
                        .store
                    ...
                    if crate::fleet::import::refresh_failure_status_clears_next_refresh_at(status) {
                        let cleared = self
                            .store
