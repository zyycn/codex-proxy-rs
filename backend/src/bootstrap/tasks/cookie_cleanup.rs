...
pub struct CookieCleanupTask {
    task: CleanupTask<PgCookieStore>,
}

impl CookieCleanupTask {
    /// 构造默认 Cookie 清理任务。
    pub fn new(store: PgCookieStore) -> Self {
        Self {
    ...
    /// 执行一次清理。
    pub async fn cleanup_once(&self) -> Result<u64, PgCookieStoreError> {
        self.task.cleanup_once().await
    }

    /// 在指定时间点执行一次清理。
    pub async fn cleanup_once_at(&self, now: DateTime<Utc>) -> Result<u64, PgCookieStoreError> {
        self.task.cleanup_once_at(now).await
    }
}

impl ExpiredCleanupStore for PgCookieStore {
    type Error = PgCookieStoreError;
