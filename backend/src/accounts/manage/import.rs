...
impl AccountManageService {
    ...
    ) -> Result<ImportedAccounts, AccountManageError> {
        let parsed = crate::accounts::import::parse_account_import_payload(&data)
            .map_err(|_| AccountManageError::NoImportableAccounts)?;
        let source_format = parsed.source.as_str();
        let entries = parsed.entries;
        if entries.is_empty() {
            return Err(AccountManageError::NoImportableAccounts);
        }
    ...
    async fn import_entry(
        &self,
        entry: crate::accounts::import::AccountImportEntry,
        source: crate::accounts::import::AccountImportSource,
    ) -> Result<ImportedAccountState, AccountManageError> {
        ...
        let label = crate::accounts::import::normalize_label(entry.label);
        if label
            .as_ref()
        ...
        let access_token_expires_at = entry
            ...
            .as_deref()
            .map(crate::accounts::import::parse_account_import_datetime)
            .transpose()
            .map_err(|_| AccountManageError::InvalidAccessTokenExpiresAt)?;
        ...
        let parsed_status =
            crate::accounts::import::parse_account_import_status(entry.status.as_deref())
                .map_err(|error| AccountManageError::InvalidStatus(error.to_string()))?;
        let mut status = crate::accounts::import::normalized_imported_account_status(
            parsed_status,
            source,
        ...
        let mut plan_type = claims
            ...
            .or_else(|| crate::accounts::import::normalize_nonempty(entry.plan_type));
        ...
        let email = claims
            ...
            .or_else(|| crate::accounts::import::normalize_nonempty(entry.email.clone()));
        let chatgpt_account_id = claims
            .and_then(|claims| claims.account_id.as_deref())
            .or_else(|| {
                crate::accounts::import::normalize_nonempty_str(entry.account_id.as_deref())
            });
        let chatgpt_user_id = claims
            .and_then(|claims| claims.user_id.as_deref())
            .or_else(|| crate::accounts::import::normalize_nonempty_str(entry.user_id.as_deref()));
    ...
    async fn resolve_import_tokens(
    ...
    ) -> Result<Option<ResolvedImportTokens>, AccountManageError> {
        let refresh_token = crate::accounts::import::normalize_nonempty(refresh_token);
        let Some(access_token) = crate::accounts::import::normalize_nonempty(
            token
                .as_deref()
        ...
        if let Ok(claims) =
            crate::accounts::refresh::manual_account_claims(&access_token, Utc::now())
        {
            return Ok(Some(ResolvedImportTokens {
                access_token,
                refresh_token,
    ...
    async fn import_target_account_id(
        ...
        account_id: Option<&str>,
        user_id: Option<&str>,
    ) -> Result<String, AccountManageError> {
        let provided_id =
            crate::accounts::import::normalize_nonempty_str(id).map(ToString::to_string);
        ...
        let chatgpt_account_id = crate::accounts::import::normalize_nonempty_str(account_id);
        let chatgpt_user_id = crate::accounts::import::normalize_nonempty_str(user_id);
        ...
        Ok(provided_id.unwrap_or_else(|| crate::accounts::import::normalized_account_id(None)))
    }
}
