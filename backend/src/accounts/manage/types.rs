...
impl From<StoredAccountMetadata> for ManagedAccount {
...
pub struct AccountRefreshResult {
    ...
    pub outcome: AccountRefreshOutcome,
...
pub struct AccountHealthCheck {
    pub results: Vec<AccountRefreshResult>,
}

impl AccountHealthCheck {
    pub fn alive(&self) -> usize {
        self.results
            .iter()
            .filter(|result| result.outcome == AccountRefreshOutcome::Alive)
            .count()
    }
    ...
    pub fn dead(&self) -> usize {
        self.results
            .iter()
            .filter(|result| result.outcome == AccountRefreshOutcome::Dead)
            .count()
    }
    ...
    pub fn skipped(&self) -> usize {
        self.results
            .iter()
            .filter(|result| result.outcome == AccountRefreshOutcome::Skipped)
            .count()
    }
}
...
pub(super) struct ManualCreateTokens {
    ...
    pub(super) claims: crate::accounts::refresh::ManualAccountClaims,
}
...
pub(super) struct ResolvedImportTokens {
    pub(super) access_token: String,
    pub(super) refresh_token: Option<String>,
    pub(super) claims: Option<crate::accounts::refresh::ManualAccountClaims>,
}
...
pub(super) fn stored_to_admin_metadata(s: crate::accounts::store::StoredAccount) -> ManagedAccount {
    ManagedAccount::from(crate::accounts::store::StoredAccountMetadata {
        id: s.id,
        email: s.email,
...
pub(super) fn import_usage_string(usage: &serde_json::Value, key: &str) -> Option<String> {
    usage
        .get(key)
        .and_then(serde_json::Value::as_str)
        .and_then(|value| crate::accounts::import::normalize_nonempty_str(Some(value)))
...
pub(super) fn parse_account_status(status: &str) -> Result<AcctStatus, AccountManageError> {
    match status.trim().to_ascii_lowercase().as_str() {
        ...
        _ => Err(AccountManageError::InvalidStatus(
