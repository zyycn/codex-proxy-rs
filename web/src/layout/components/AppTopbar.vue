...
<template>
  <div class="relative z-30 flex h-11 items-start gap-3.5">
    ...
    >
      ...
      <ChevronDown class="text-(--cp-text-secondary) transition-transform duration-200 group-hover:rotate-180" :size="16" />
    </button>
    ...
    <div class="relative ml-0.5">
      ...
      >
        <span class="inline-flex size-7 items-center justify-center rounded-full bg-(--cp-normal) text-xs leading-[1.15] font-bold text-white">A</span>
        <span class="grid gap-1.25 pl-3 text-left">
          <strong class="text-[13px] leading-[1.15] font-bold text-(--cp-text-primary)">admin</strong>
          <span class="text-[11px] leading-[1.15] font-[650] text-(--cp-text-secondary)">管理员</span>
        </span>
        <ChevronUp v-if="accountMenuOpen" class="text-(--cp-text-secondary) transition-transform duration-200 group-hover:rotate-180" :size="16" />
        <ChevronDown v-else class="text-(--cp-text-secondary) transition-transform duration-200 group-hover:rotate-180" :size="16" />
      </button>
      ...
      >
        <div v-if="accountMenuOpen" data-account-menu class="absolute right-0 top-14 h-76 w-80 origin-top-right rounded-[18px] bg-white pt-4.5 shadow-(--cp-shadow-popover)">
          <div class="mx-4.5 grid h-11 grid-cols-[44px_minmax(0,1fr)_64px] items-center gap-3">
          <span class="inline-flex size-11 items-center justify-center rounded-full bg-(--cp-normal) text-sm leading-[1.15] font-bold text-white">A</span>
          <span class="grid gap-1.25">
            <strong class="text-[15px] leading-[1.15] font-bold text-(--cp-text-primary)">admin</strong>
            <span class="text-xs leading-[1.15] font-[650] text-(--cp-text-secondary)">管理员</span>
          </span>
          <span class="inline-flex h-7 w-16 items-center gap-1.5 rounded-full bg-(--cp-success-bg) px-3">
            <i class="size-1.5 rounded-full bg-(--cp-success)" />
            <span class="text-xs leading-[1.15] font-[650] text-(--cp-success-text)">在线</span>
          </span>
          </div>

          <div class="mx-4 mt-3.5 grid h-11 w-72 grid-cols-2 rounded-xl bg-(--cp-bg-subtle) px-4 py-2">
          <span class="grid gap-1.25">
            <small class="text-[11px] leading-[1.15] font-semibold text-(--cp-text-secondary)">MFA</small>
            <strong class="text-[13px] leading-[1.15] font-bold text-(--cp-text-primary)">已开启</strong>
          </span>
          <span class="grid gap-1.25 pl-4.5">
            <small class="text-[11px] leading-[1.15] font-semibold text-(--cp-text-secondary)">会话</small>
            <strong class="text-[13px] leading-[1.15] font-bold text-(--cp-text-primary)">1 台</strong>
          </span>
          </div>
