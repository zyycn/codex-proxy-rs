...
<template>
  <BaseCard as="article" :padded="false" radius-class="rounded-2xl" class="h-[154px] w-full px-6 pt-5">
    ...
    <strong class="mt-2.5 block w-full font-mono text-[28px] leading-[1.05] font-[780] tabular-nums text-[var(--cp-text-primary)]">
      {{ metric.value }}
    </strong>

    <div class="mt-[16.6px] grid h-[30px] w-full grid-cols-2 items-center rounded-[10px] bg-[var(--cp-bg-subtle)] px-3">
      <span class="inline-flex min-w-0 w-full items-center justify-start gap-3">
        <span class="shrink-0 text-[11px] leading-[1.15] font-[650] text-[var(--cp-text-muted)]">{{ metric.details[0]?.label }}</span>
        <b class="min-w-0 truncate font-mono text-[12px] leading-[1.15] font-bold tabular-nums" :class="metric.details[0]?.tone ? detailToneClasses[metric.details[0].tone] : undefined">
          {{ metric.details[0]?.value }}
        </b>
      </span>
      <span class="inline-flex min-w-0 w-full items-center justify-start gap-3">
        <span class="shrink-0 text-[11px] leading-[1.15] font-[650] text-[var(--cp-text-muted)]">{{ metric.details[1]?.label }}</span>
        <b class="min-w-0 truncate font-mono text-[12px] leading-[1.15] font-bold tabular-nums" :class="metric.details[1]?.tone ? detailToneClasses[metric.details[1].tone] : undefined">
