...
impl AccountOAuthService {
    ...
    async fn authorize(&self) -> Result<OAuthAuthorizeResult, AdminAccountError> {
        let now = Utc::now();
        ...
        let mut auth_url =
            Url::parse(OPENAI_OAUTH_AUTHORIZE_ENDPOINT).map_err(|_| AdminAccountError::Import)?;
        auth_url
            .query_pairs_mut()
    ...
    async fn exchange(
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|error| AdminAccountError::OAuthCodeExchange(error.to_string()))?;
        ...
        if !status.is_success() {
            return Err(AdminAccountError::OAuthCodeExchange(exchange_error_text(
                status, &body,
            )));
        }

        let tokens = serde_json::from_str::<AccountOAuthTokens>(&body)
            .map_err(|_| AdminAccountError::OAuthCodeExchange("invalid token response".into()))?;
        if tokens.access_token.trim().is_empty() {
            return Err(AdminAccountError::OAuthCodeExchange(
    ...
    async fn session(&self, session_id: &str) -> Result<AccountOAuthSession, AdminAccountError> {
        let now = Utc::now();
        let mut sessions = self.sessions.lock().await;
        sessions.retain(|_, session| session.expires_at > now);
        sessions
            .get(session_id)
            .cloned()
            .ok_or(AdminAccountError::OAuthSessionInvalid)
    }
...
impl AdminAccountService {
    pub async fn oauth_authorize(&self) -> Result<OAuthAuthorizeResult, AdminAccountError> {
...
fn oauth_callback_parts(input: &OAuthExchangeInput) -> Result<(String, String), AdminAccountError> {
    if let Some(callback_url) = input.callback_url.as_deref() {
        let (code, state) = callback_url_parts(callback_url)?;
        return Ok((code, state));
    }

    let code = normalized(input.code.as_deref()).ok_or(AdminAccountError::OAuthCallbackInvalid)?;
    let state =
        normalized(input.state.as_deref()).ok_or(AdminAccountError::OAuthCallbackInvalid)?;
    Ok((code, state))
}

fn callback_url_parts(callback_url: &str) -> Result<(String, String), AdminAccountError> {
    let url =
        Url::parse(callback_url.trim()).map_err(|_| AdminAccountError::OAuthCallbackInvalid)?;
    let mut code = None;
    let mut state = None;
    ...
    code.zip(state)
        .ok_or(AdminAccountError::OAuthCallbackInvalid)
}
