...
mod tests {
    ...
    #[test]
    fn calculate_cost_should_use_gpt55_standard_prices() {
        let actual = calculate_cost(100_000, 100_000, 0, "gpt-5.5", None);

        assert_eq!(actual, 3.5);
    }

    #[test]
    fn calculate_cost_should_use_priority_prices_when_available() {
        let actual = calculate_cost(100_000, 100_000, 0, "gpt-5.5", Some("priority"));

        assert_eq!(actual, 8.75);
    }

    #[test]
    fn calculate_cost_should_charge_cached_input_at_cache_read_price() {
        let actual = calculate_cost(1_000, 1_000, 500, "gpt-5.5", None);

        assert!((actual - 0.03275).abs() < 1e-12);
    }

    #[test]
    fn calculate_cost_should_apply_long_context_prices() {
        let actual = calculate_cost(300_000, 1_000, 0, "gpt-5.5", None);

        assert!((actual - 3.045).abs() < 1e-12);
    }

    #[test]
    fn calculate_cost_should_normalize_codex_model_names() {
        let actual = calculate_cost(100_000, 100_000, 0, "codex-auto-review", None);

        assert_eq!(actual, 1.75);
    }
}
