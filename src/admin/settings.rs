...
impl SettingsService {
    pub fn new(config: Arc<AppConfig>, local_config_path: impl Into<PathBuf>) -> Self {
        Self {
    ...
    ) -> Result<Arc<AppConfig>, SettingsServiceError> {
    ...
    async fn write_overlay(&self, config: &AppConfig) -> Result<(), SettingsServiceError> {
        ...
        let content = serde_yml::to_string(&overlay).map_err(SettingsServiceError::Serialize)?;
