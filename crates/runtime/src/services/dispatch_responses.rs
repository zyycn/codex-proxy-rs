...
pub(super) fn is_rate_limit_upstream_error(error: &CodexClientError) -> bool {
    matches!(
        error,
...
pub(super) fn is_quota_exhausted_upstream_error(error: &CodexClientError) -> bool {
    matches!(
        error,
...
pub(super) fn is_auth_upstream_error(error: &CodexClientError) -> bool {
    matches!(
        error,
...
pub(super) fn is_cloudflare_challenge_upstream_error(error: &CodexClientError) -> bool {
    matches!(
        error,
        CodexClientError::Upstream { status, body, .. }
...
pub(super) fn is_cloudflare_path_block_upstream_error(error: &CodexClientError) -> bool {
    matches!(
        error,
        CodexClientError::Upstream { status, body, .. }
...
pub(super) fn is_model_unsupported_upstream_error(error: &CodexClientError) -> bool {
    matches!(
        error,
        CodexClientError::Upstream { status, body, .. }
...
pub(super) fn is_banned_upstream_error(error: &CodexClientError) -> bool {
    matches!(
        error,
        CodexClientError::Upstream { status, body, .. }
...
pub(super) fn auth_failure_account_status(error: &CodexClientError) -> AccountStatus {
    match error {
...
pub(super) fn upstream_error_body(error: &CodexClientError) -> String {
    match error {
...
pub(super) fn cloudflare_challenge_error_message() -> &'static str {
    "Upstream blocked the request (Cloudflare challenge)"
}

pub(super) fn cloudflare_path_block_error_message() -> &'static str {
    "Upstream blocked the request (Cloudflare path-block)"
}

pub(super) fn rate_limit_cooldown_until(
...
pub(super) struct ResponseEventRecord<'a> {
    pub(super) logs: &'a AdminLogService,
...
pub(super) struct ChatDispatchErrorEventRecord<'a> {
    pub(super) logs: &'a AdminLogService,
    pub(super) request_id: &'a str,
    ...
    pub(super) error: &'a ChatDispatchError,
}

pub(super) async fn record_response_event(mut record: ResponseEventRecord<'_>) {
...
pub(super) async fn record_chat_dispatch_error_event(record: ChatDispatchErrorEventRecord<'_>) {
...
async fn record_response_upstream_error_event(record: ResponseUpstreamErrorEventRecord<'_>) {
    record_response_event(ResponseEventRecord {
        ...
        status_code: i64::from(upstream_error_http_status(record.error)),
...
pub(super) fn requested_response_transport(
    request: &CodexResponsesRequest,
...
fn chat_dispatch_error_metadata(error: &ChatDispatchError, transport: Option<&str>) -> Value {
    let mut metadata = dispatch_error_metadata(error, false, false, transport);
    ...
    match error {
        ...
        ChatDispatchError::Upstream(error) => {
            insert_upstream_dispatch_metadata(object, error);
        }
        ChatDispatchError::NoActiveAccount => {
            insert_failure_class_metadata(object, "no_active_account");
        }
        ChatDispatchError::AccountStore => {
            insert_failure_class_metadata(object, "account_store");
        }
        ChatDispatchError::InvalidSse(_) => {
            insert_failure_class_metadata(object, "invalid_sse");
        }
        ChatDispatchError::EmptyUpstreamResponse => {
            insert_failure_class_metadata(object, "empty_upstream_response");
        }
    }
    metadata
}
...
fn response_dispatch_error_metadata(
    ...
    let mut metadata = dispatch_error_metadata(error, stream, compact, transport);
    let Some(object) = metadata.as_object_mut() else {
        return metadata;
    };
    ...
    match error {
        ...
        ResponseDispatchError::MissingCompleted => {
            insert_failure_class_metadata(object, "missing_completed");
        }
        ResponseDispatchError::EmptyUpstreamResponse => {
            insert_failure_class_metadata(object, "empty_upstream_response");
        }
        ResponseDispatchError::Failed(failure) => {
            insert_failure_class_metadata(object, "response_failed");
            object.insert(
...
fn insert_failure_class_metadata(object: &mut Map<String, Value>, failure_class: &str) {
    object.insert(
...
fn insert_upstream_dispatch_metadata(object: &mut Map<String, Value>, error: &CodexClientError) {
    insert_failure_class_metadata(object, "upstream");
    if let CodexClientError::Upstream {
        status,
        body,
        retry_after_seconds,
...
fn insert_exhausted_dispatch_metadata(
    ...
    insert_failure_class_metadata(object, failure_class);
    object.insert(
...
pub(super) fn upstream_error_http_status(error: &CodexClientError) -> u16 {
    match error {
...
pub(super) fn backend_transport_name(transport: CodexBackendTransport) -> &'static str {
    match transport {
