...
use codex_proxy_adapters::sqlite::admin_sessions::{
    SqliteAdminSessionStore, SqliteAdminSessionStoreResult,
};
...
impl SessionCleanupTask {
    ...
    /// 执行一次清理。
    pub async fn cleanup_once(&self) -> SqliteAdminSessionStoreResult<u64> {
        self.cleanup_once_at(Utc::now()).await
    }

    /// 在指定时间点执行一次清理。
    pub async fn cleanup_once_at(&self, now: DateTime<Utc>) -> SqliteAdminSessionStoreResult<u64> {
