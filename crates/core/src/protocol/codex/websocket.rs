//! Codex WebSocket 帧类型与纯编解码。

use serde::{ser::SerializeMap, Deserialize, Serialize};
use serde_json::{Map, Value};

use crate::protocol::codex::responses::CodexResponsesRequest;
use crate::protocol::codex::sse::encode_sse_event;
use crate::serving::responses::{http_sse_fallback_allowed, transport_for_request, CodexTransport};
...
/// Build a WebSocket audit artifact for a single opening attempt.
...
/// Diff two WebSocket audit artifacts by the fields used for parity checks.
...
fn opening_request_line(opening: &Option<OpeningAuditSnapshot>) -> Value {
    opening
        .as_ref()
        .map(|opening| serde_json::json!(opening.request_line))
...
fn opening_header_order(opening: &Option<OpeningAuditSnapshot>) -> Value {
    opening
        .as_ref()
        .map(|opening| serde_json::json!(opening.header_order))
        .unwrap_or(Value::Null)
}
...
fn opening_header_value(opening: &Option<OpeningAuditSnapshot>, name: &str) -> Value {
    opening
        ...
        .map(|header| serde_json::json!(header.value))
        .unwrap_or(Value::Null)
}
...
fn payload_top_level_keys(payload: &Option<PayloadAuditSnapshot>) -> Value {
    payload
        .as_ref()
        .map(|payload| serde_json::json!(payload.top_level_keys))
        .unwrap_or(Value::Null)
}
...
fn error_classification(error: &Option<WebSocketAuditErrorSnapshot>) -> Value {
    error
        .as_ref()
        .map(|error| serde_json::json!(error.classification))
        .unwrap_or(Value::Null)
}
