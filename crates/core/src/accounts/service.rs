...
impl AccountService {
    /// 判断账号是否可参与请求调度。
    pub fn can_serve(account: &Account) -> bool {
        account.status == AccountStatus::Active && !account.quota_limit_reached
    }

    /// 判断账号在指定时间是否处于可服务的基础状态。
    pub fn can_serve_at(account: &Account, now: DateTime<Utc>, skip_quota_limited: bool) -> bool {
        account.status == AccountStatus::Active
            && Self::quota_available_at(account, now, skip_quota_limited)
            && Self::cloudflare_available_at(account, now)
    }
