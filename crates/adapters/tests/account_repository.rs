...
use codex_proxy_adapters::sqlite::accounts::{NewAccount, SqliteAccountStore, UsageDelta};
...
async fn account_repository_should_update_quota_json_and_fetched_at() {
    ...
    let updated = repo
        .update_quota_json("acct_a", r#"{"rate_limit":{"limit_reached":false}}"#)
        .await
        .unwrap();
    ...
    let row: (String, Option<String>) =
        sqlx::query_as("select quota_json, quota_fetched_at from accounts where id = ?")
            .bind("acct_a")
            .fetch_one(&pool)
            .await
            .unwrap();
    ...
    assert_eq!(
        (updated, missing_updated, row.0, row.1.is_some()),
        (
            true,
            false,
            r#"{"rate_limit":{"limit_reached":false}}"#.to_string(),
            true
        )
    );
}
