...
fn custom_ca_should_report_environment_cache_key_consistently() {
    ...
    if let Ok(case) = std::env::var(CASE_ENV) {
        let expected = match case.as_str() {
            "unset" => None,
            "ssl_cert_file" => Some(format!("{}={SSL_CERT_PATH}", client::SSL_CERT_FILE_ENV)),
            "codex_ca_priority" => Some(format!("{}={CODEX_CA_PATH}", client::CODEX_CA_CERT_ENV)),
            _ => panic!("unknown custom CA cache key test case: {case}"),
        };

        assert_eq!(client::custom_ca_env_cache_key(), expected);
        return;
    }
    ...
    let cases = [
        ...
        (
            "ssl_cert_file",
            None,
            Some((client::SSL_CERT_FILE_ENV, SSL_CERT_PATH)),
        ),
        (
            "codex_ca_priority",
            Some((client::CODEX_CA_CERT_ENV, CODEX_CA_PATH)),
            Some((client::SSL_CERT_FILE_ENV, SSL_CERT_PATH)),
        ),
    ];
    ...
    for (case, codex_ca, ssl_cert_file) in cases {
        ...
        command
            ...
            .arg("--nocapture")
            .env(CASE_ENV, case)
            .env_remove(client::CODEX_CA_CERT_ENV)
            .env_remove(client::SSL_CERT_FILE_ENV);
