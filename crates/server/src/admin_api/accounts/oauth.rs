...
pub async fn auth_device_poll(
    ...
    Extension(request_id): Extension<RequestId>,
    headers: HeaderMap,
    axum::extract::Path(device_code): axum::extract::Path<String>,
) -> Result<impl IntoResponse, AdminError> {
    let request_id = request_id.as_str().to_string();
    require_admin_session(&state, &headers, &request_id).await?;
...
impl From<codex_proxy_runtime::services::AdminAuthStatus> for AdminAuthStatusData {
    fn from(status: codex_proxy_runtime::services::AdminAuthStatus) -> Self {
        Self {
