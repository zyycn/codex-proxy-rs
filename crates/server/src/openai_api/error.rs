...
use codex_proxy_core::protocol::openai::responses::response_failed_sse_event as encode_response_failed_sse_event;
use codex_proxy_runtime::services::{ChatDispatchError, ResponseDispatchError};
...
#[derive(Clone, Copy)]
pub(super) enum ResponseDispatchMessageStyle {
    Standard,
    ResponsesStream,
}

#[derive(Clone, Copy)]
pub(super) enum ResponseDispatchStatusMode {
    GatewayUpstream,
    Client,
}

pub(super) struct DispatchHttpError {
    pub(super) status: StatusCode,
    pub(super) message: String,
}

pub(super) struct OpenAiErrorDetails {
    pub(super) status: StatusCode,
    pub(super) message: String,
...
pub(super) fn chat_dispatch_error_response(error: ChatDispatchError) -> Response {
    let error = chat_dispatch_openai_error(&error);
    openai_error_response(error.status, &error.message, error.error_type, error.code)
        .into_response()
}

pub(super) fn chat_stream_dispatch_error_message(error: &ResponseDispatchError) -> String {
    response_dispatch_http_error(
        error,
        ResponseDispatchStatusMode::GatewayUpstream,
        ResponseDispatchMessageStyle::Standard,
    )
    .message
}

pub(super) fn responses_dispatch_error_response(error: ResponseDispatchError) -> Response {
    match error {
...
pub(super) fn responses_compact_dispatch_error_response(error: ResponseDispatchError) -> Response {
    match error {
        ResponseDispatchError::NoActiveAccount | ResponseDispatchError::AccountStore => {
            responses_no_available_accounts_response().into_response()
        }
...
pub(super) fn responses_stream_dispatch_failed_sse_event(error: &ResponseDispatchError) -> String {
    let error = response_dispatch_http_error(
        error,
        ResponseDispatchStatusMode::GatewayUpstream,
...
/// 模型不存在。
pub(super) fn model_not_found_response() -> (StatusCode, Json<Value>) {
    openai_error_response(
...
pub(super) fn invalid_chat_completion_request_response() -> (StatusCode, Json<Value>) {
    invalid_openai_request_response("Invalid chat completion request")
}

/// Responses 请求无效。
pub(super) fn invalid_responses_request_response() -> (StatusCode, Json<Value>) {
...
pub(super) fn response_dispatch_http_error(
    error: &ResponseDispatchError,
...
fn response_dispatch_status(
    ...
    match (mode, error) {
        (ResponseDispatchStatusMode::GatewayUpstream, ResponseDispatchError::Upstream(_)) => {
