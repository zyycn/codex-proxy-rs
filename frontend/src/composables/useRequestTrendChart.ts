...
import type { RequestTrendKind, RequestTrendPoint, RequestTrendSummaryItem } from '@/views/overview/model/display'
...
export function useRequestTrendChart(options: {
  points: Readonly<Ref<RequestTrendPoint[]>>
  summary: Readonly<Ref<RequestTrendSummaryItem[]>>
  activeKind: Ref<RequestTrendKind>
  onTrendChange: (kind: RequestTrendKind) => void
}) {
  ...
  function tooltipDisplayValue(point: RequestTrendPoint | undefined, name: string, value: string) {
    if (activeKind.value === 'usage') {
      if (name === '输入')
  ...
  function summaryMarkerStyle(item: RequestTrendSummaryItem) {
  ...
  function handleTrendChange(value: string) {
    options.onTrendChange(value as RequestTrendKind)
  }
...
function trendPointProperty(point: RequestTrendPoint | undefined, key: string) {
  if (!point || !(key in point))
    return undefined
  return (point as unknown as Record<string, unknown>)[key]
}

function trendPointText(point: RequestTrendPoint | undefined, key: string) {
  const value = trendPointProperty(point, key)
  return typeof value === 'string' ? value : undefined
}

function zeroAwareRateText(
  point: RequestTrendPoint | undefined,
  valueKey: string,
  activityKey: string,
) {
...
function trendPointNumber(point: RequestTrendPoint, key: string) {
  const value = trendPointProperty(point, key)
