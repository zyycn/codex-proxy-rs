...
export function getProxies(params: { page: number, pageSize: number, search?: string }) {
  return request<ProxyPage>({ url: '/api/admin/proxies', method: 'GET', params })
}

export function createProxy(data: { name: string, proxyUrl: string }) {
  return request<ProxyMutation>({ url: '/api/admin/proxies/create', method: 'POST', data })
}

export function updateProxy(data: { id: string, revision: number, name: string, proxyUrl?: string }) {
  return request<ProxyMutation>({ url: '/api/admin/proxies/update', method: 'POST', data })
}

export function deleteProxy(data: { id: string, revision: number }) {
  return request<{ configRevision: number }>({ url: '/api/admin/proxies/delete', method: 'POST', data })
}

export function testProxy(data: { id: string, revision: number }) {
  return request<OutboundProxyRecord>({ url: '/api/admin/proxies/test', method: 'POST', data, timeout: 25000 })
}
