...
<template>
  <BaseCard as="article" padding="compact" class="relative h-38.5 w-full">
    <div class="flex items-start gap-3">
      <BaseMotionIcon
        aria-hidden="true"
        class="inline-flex size-8.5 shrink-0 items-center justify-center rounded-cp-overlay"
        :class="metricToneIconClasses[metric.tone]"
      >
        <component :is="metric.icon" :size="18" />
      </BaseMotionIcon>
      <span class="mt-1 text-[13px] leading-[1.15] font-emphasis text-cp-secondary">{{ metric.title }}</span>
    </div>

    <div class="mt-3.25 flex h-7.75 items-end gap-2">
      <strong
        class="font-mono text-[28px] leading-[1.05] font-heavy tabular-nums text-cp-primary"
      >
        <AnimatedMetricValue
          :value="metric.value"
    ...
    <div
      class="mt-3 grid h-7.5 w-full grid-cols-[minmax(0,1fr)_minmax(0,1fr)] items-center rounded-cp-overlay bg-cp-subtle px-3"
    >
      <span
        class="inline-grid min-w-0 w-full grid-cols-[auto_minmax(0,1fr)] items-baseline gap-2.5"
      >
        <span class="shrink-0 text-[11px] leading-none font-emphasis text-cp-muted-text">{{ metric.details[0]?.label }}</span>
        <b
      ...
      >
        <span class="shrink-0 text-[11px] leading-none font-emphasis text-cp-muted-text">{{ metric.details[1]?.label }}</span>
        <b
          class="min-w-0 truncate font-mono text-xs leading-none font-bold tabular-nums"
