...
const billingItems = computed(() => {
  ...
  return [
    ...
    { label: '倍率', value: value.multiplierDisplay },
    { label: '总费用', value: value.totalAmountDisplay },
    { label: '标准费用', value: value.standardAmountDisplay },
  ]
})
...
const amountItems = computed(() => {
  ...
  return [
    { label: '输入费用', value: value.inputAmountDisplay },
    { label: '输出费用', value: value.outputAmountDisplay },
    { label: '输入单价', value: value.inputPriceDisplay },
    { label: '输出单价', value: value.outputPriceDisplay },
    { label: '缓存读取费用', value: value.cacheReadAmountDisplay },
    { label: '缓存写入费用', value: value.cacheWriteAmountDisplay },
    { label: '缓存写入单价', value: value.cacheWritePriceDisplay },
  ]
})
</script>
    ...
    <UsageDetailPopover v-if="billing" title="计费明细" trigger-label="查看费用明细">
      <div class="grid gap-1.5 text-cp-text-secondary">
        <div v-for="item in amountItems" :key="item.label" class="grid grid-cols-[auto_minmax(0,1fr)] items-center gap-4">
          ...
          <span class="justify-self-end whitespace-nowrap font-mono font-heavy text-cp-text">
            {{ item.value }}
          </span>
        </div>
      </div>
