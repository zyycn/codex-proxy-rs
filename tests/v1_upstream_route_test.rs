...
async fn build_imported_app_with_accounts_and_token_refresher<C>(
    base_url: String,
    accounts: &[ImportAccount],
    token_refresher: C,
) -> ImportedApp
...
async fn v1_responses_non_stream_should_return_upstream_error_when_sse_error_event_arrives() {
    ...
    let sse_body = concat!(
        "event: error\n",
        "data: {\"error\":{\"message\":\"model overloaded\",\"type\":\"server_error\",\"code\":\"server_error\"}}\n",
        "\n",
    );
    Mock::given(method("POST"))
        .and(path("/codex/responses"))
...
async fn v1_responses_non_stream_should_return_upstream_error_when_response_failed_arrives() {
    ...
    let sse_body = concat!(
        ...
        "data: {\"response\":{\"id\":\"resp_failed\",\"object\":\"response\",\"status\":\"failed\",\"error\":{\"code\":\"rate_limit_exceeded\",\"message\":\"quota reached\"}}}\n",
        "\n",
    );
    Mock::given(method("POST"))
        .and(path("/codex/responses"))
...
async fn v1_responses_stream_should_passthrough_error_event_and_log_failure() {
    ...
    let sse_body = concat!(
        "event: error\n",
        "data: {\"error\":{\"message\":\"upstream stream failed\",\"type\":\"server_error\",\"code\":\"upstream_error\"}}\n",
        "\n",
    );
    Mock::given(method("POST"))
        .and(path("/codex/responses"))
...
async fn v1_responses_stream_should_passthrough_response_failed_and_log_failure() {
    ...
    let sse_body = concat!(
        "event: response.failed\n",
        "data: {\"response\":{\"id\":\"resp_stream_failed\",\"object\":\"response\",\"status\":\"failed\",\"error\":{\"code\":\"server_error\",\"message\":\"stream failed\"}}}\n",
        "\n",
    );
    Mock::given(method("POST"))
        .and(path("/codex/responses"))
...
async fn v1_responses_previous_response_id_websocket_429_should_not_retry_different_account() {
    ...
    let server = tokio::spawn(async move {
        reject_next_websocket_upgrade(
            ...
            "Too Many Requests",
            Some(77),
            r#"{"error":{"message":"history account rate limited"}}"#,
        )
        .await;
    });
    let imported = build_imported_app_with_accounts(
...
async fn v1_responses_websocket_without_history_should_return_429_when_fallback_accounts_exhausted()
{
    ...
    let server = tokio::spawn(async move {
        reject_next_websocket_upgrade(
            ...
            "Too Many Requests",
            Some(11),
            r#"{"error":{"message":"first account limited"}}"#,
        )
        .await;
        reject_next_websocket_upgrade(
            &listener,
            ...
            "Too Many Requests",
            Some(22),
            r#"{"error":{"message":"second account limited"}}"#,
        )
        .await;
    });
    let imported = build_imported_app_with_accounts(
