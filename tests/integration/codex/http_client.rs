...
fn endpoints_should_join_backend_paths() {
    assert_eq!(
        codex_proxy_rs::codex::transport::endpoint_url(
    ...
    assert_eq!(
        codex_proxy_rs::codex::transport::endpoint_request_path(
...
fn custom_ca_should_report_environment_cache_key_consistently() {
    ...
    if let Ok(case) = std::env::var(CASE_ENV) {
        let expected = match case.as_str() {
            ...
            "ssl_cert_file" => Some(format!(
                "{}={SSL_CERT_PATH}",
                codex_proxy_rs::codex::transport::SSL_CERT_FILE_ENV
            )),
            "codex_ca_priority" => Some(format!(
                "{}={CODEX_CA_PATH}",
                codex_proxy_rs::codex::transport::CODEX_CA_CERT_ENV
            )),
            _ => panic!("unknown custom CA cache key test case: {case}"),
        };

        assert_eq!(
            codex_proxy_rs::codex::transport::custom_ca_env_cache_key(),
            expected
        );
        return;
    }
    ...
    for (case, codex_ca, ssl_cert_file) in cases {
        ...
        command
            ...
            .arg("--nocapture")
            .env(CASE_ENV, case)
            .env_remove(codex_proxy_rs::codex::transport::CODEX_CA_CERT_ENV)
            .env_remove(codex_proxy_rs::codex::transport::SSL_CERT_FILE_ENV);
