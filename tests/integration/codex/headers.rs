...
async fn codex_backend_client_should_send_desktop_headers_and_capture_response_metadata() {
    ...
    assert_eq!(
        response.usage,
        Some(codex_proxy_rs::codex::protocol::events::TokenUsage {
            input_tokens: 2,
            output_tokens: 3,
            cached_tokens: 1,
            reasoning_tokens: 0,
...
async fn codex_backend_client_models_should_use_original_auxiliary_headers() {
    use codex_proxy_rs::codex::transport::{CodexModelCatalogClient, CodexModelCatalogRequest};
...
async fn codex_backend_client_should_send_compact_headers_in_fingerprint_order() {
    ...
    client
        .create_compact_response(
            &codex_proxy_rs::codex::protocol::responses::CodexCompactRequest {
