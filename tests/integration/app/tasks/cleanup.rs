...
async fn cookie_cleanup_task_should_delete_only_expired_cookie_rows() {
    ...
    let deleted = codex_proxy_rs::app::tasks::cookie_cleanup::CookieCleanupTask::new(store)
        .cleanup_once_at(now)
        .await
...
async fn session_cleanup_task_should_delete_only_expired_sessions() {
    ...
    let deleted = codex_proxy_rs::app::tasks::session_cleanup::SessionCleanupTask::new(store, 3600)
        .cleanup_once_at(now)
        .await
...
async fn session_affinity_cleanup_task_should_delete_only_expired_affinities() {
    ...
    let deleted =
        codex_proxy_rs::app::tasks::session_affinity_cleanup::SessionAffinityCleanupTask::new(
            store, 3600,
        )
        .cleanup_once_at(now)
        .await
