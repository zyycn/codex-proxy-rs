...
use codex_proxy_rs::{
    ...
    config::AppConfig,
    gateway::dispatch::session_affinity::{
...
#[path = "chat_upstream/chat_routes.rs"]
mod openai_chat_routes;
#[path = "chat_upstream/compact_routes.rs"]
mod openai_compact_routes;
#[path = "chat_upstream/responses_http.rs"]
mod openai_responses_http;
#[path = "chat_upstream/responses_recovery.rs"]
mod openai_responses_recovery;
#[path = "chat_upstream/responses_websocket.rs"]
mod openai_responses_websocket;
#[path = "chat_upstream/usage_logging.rs"]
...
async fn test_app_with_two_accounts_and_affinity(
    ...
    state
        .services
        .session_affinity
        .restore_from_repository(now + Duration::minutes(1))
        .await
...
async fn test_app_with_two_accounts_and_affinity_status(
    ...
    let db = dir.path().join(format!(
        "openai-responses-affinity-{affinity_account_status}.sqlite"
    ));
    let url = format!("sqlite://{}", db.display());
    let pool = connect_sqlite(&url).await.unwrap();
    let secret_box = SecretBox::new([83u8; 32]);
    ...
    SqliteSessionAffinityStore::new(pool.clone())
        .upsert(
            "resp_affinity_risk",
            &SessionAffinityEntry {
                account_id: "acct_affinity".to_string(),
                conversation_id: "conv_affinity_risk".to_string(),
                turn_state: Some("turn_affinity_risk".to_string()),
                instructions_hash: None,
                input_tokens: None,
                function_call_ids: Vec::new(),
                variant_hash: None,
