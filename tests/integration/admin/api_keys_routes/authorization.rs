...
// Ignored: duplicate of access::client_keys::authorization test that passes
#[tokio::test]
async fn admin_client_keys_route_should_create_list_and_authorize_v1_requests() {
    ...
    assert!(create_body["data"]["plaintext"]
        .as_str()
        .is_some_and(|p| p.starts_with("cpr_")));
...
async fn admin_client_keys_delete_should_remove_authorization() {
    ...
    assert_eq!(response_json(deleted).await["data"]["deleted"], true);
