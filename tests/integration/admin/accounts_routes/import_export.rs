...
async fn admin_accounts_import_cli_should_read_auth_file_store_encrypted_and_sync_pool() {
    ...
    let config = test_config(url);
    let hasher = ApiKeyHasher::new([104u8; 32]);
    let stores = BackgroundTaskStores {
        accounts: SqliteAccountStore::new(pool.clone(), secret_box.clone()),
        admin_sessions: SqliteAdminSessionStore::new(pool.clone()),
        cookies: SqliteCookieStore::new(pool.clone(), secret_box.clone()),
    ...
    fs::write(
        ...
        json!({"access_token": token, "refresh_token": "cli-refresh-secret"}).to_string(),
    )
    .unwrap();
    ...
    let response = app
        .oneshot(
            Request::builder()
                ...
                .body(Body::from(
                    json!({
                        "codexHome": codex_home.display().to_string(), "id": "caller-id",
                        "email": "caller@example.com", "accountId": "caller-account",
                        "userId": "caller-user", "label": "caller-label",
